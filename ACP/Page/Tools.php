...
final readonly class Tools
{
    ...
    private function backup(): void
    {
        ...
        $this->page->addContentBox('Backup Forum', $this->page->render('tools/backup.html'));
    }
    ...
    private function createForumBackup(): void
    {
        ...
        foreach ($models as $model) {
            ...
            $sqlFileLines[] = $this->databaseUtils->createTableQueryFromModel($modelInstance) . ';';
    ...
    private function outputZipFile(string $fileContents): void
    {
        ...
        header(
            'Content-Disposition: attachment;filename="' . $this->database->getPrefix() . gmdate('Y-m-d_His') . '.zip"',
        );
        ...
        $zipArchive->addGlob($boardPath . "{{$globStarStarIsNotSupportedWTF}}/*.*", GLOB_BRACE, [
    ...
    private function outputTextFile(string $fileContents): void
    {
        ...
        header(
            'Content-Disposition: attachment;filename="' . $this->database->getPrefix() . gmdate('Y-m-d_His') . '.sql"',
        );
    ...
    private function viewErrorLog(): void
    {
        ...
        if ($this->fileSystem->getFileInfo($logPath, true)->isReadable()) {
            ...
            $last100Lines = htmlspecialchars(implode(PHP_EOL, $this->fileSystem->tail($logFile, 100)));
            $contents = <<<HTML
                <label for="errorlog">
