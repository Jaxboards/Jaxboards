export default class BetterSelect extends HTMLElement {
  ...
  connectedCallback() {
    ...
    search.addEventListener("keydown", (event) => {
      switch (event.key) {
        case "ArrowUp":
          this.shadow
            .querySelector<HTMLButtonElement>(".results button:last-child")
            ?.focus();
          break;
        case "ArrowDown":
          this.shadow
            .querySelector<HTMLButtonElement>(".results button:first-child")
            ?.focus();
          break;
        default:
          break;
  ...
  getOptions(searchTerm = ""): HTMLElement[] {
    return Array.from(this.querySelectorAll<HTMLElement>("better-option"))
      .filter((betterOption) =>
        betterOption.innerText.toLowerCase().includes(searchTerm.toLowerCase()),
      )
      .map((betterOption) => {
