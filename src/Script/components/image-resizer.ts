...
export default class ImageResizer extends Component<HTMLImageElement> {
  ...
  constructor(element: HTMLImageElement) {
    ...
    const maxWidth =
      Number.parseInt(style.width, 10) || Number.parseInt(style.maxWidth, 10);
    const maxHeight =
      Number.parseInt(style.height, 10) || Number.parseInt(style.maxHeight, 10);
    if (maxWidth && naturalWidth > maxWidth) p = maxWidth / naturalHeight;
    if (maxHeight && naturalHeight > maxHeight) p2 = maxHeight / naturalHeight;
