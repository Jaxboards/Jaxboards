...
final class Router
{
    ...
    public function route(): void
    {
        ...
        $action = $this->request->asString->both('act') ?? '';

        match (true) {
            $this->isBoardOffline() && !str_contains($path, 'login') => $this->container->get(BoardOffline::class)->route([]),
    ...
    /**
     * Routes by ?act=...
     * This is for legacy URLs and will likely be phased out.
     */
    private function routeByAct(string $action): void
    {
        $params = [];
        ...
        // Fallback to attempting to load custom page
        $this->loadCustomPage($action);
    }
    ...
    private function routeByPath(string $path = ''): void
    {
        ...
        // Exhausted all routing attempts, just go to the index
        $this->redirect('index');
    }
