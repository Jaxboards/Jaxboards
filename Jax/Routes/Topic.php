...
final class Topic implements Route
{
    ...
    public function route($params): void
    {
        ...
        $forumPerms =
            $topic !== null ? $this->fetchForumPermissions($topic) : [];
        ...
        match (true) {
            $quickReply && !$this->request->isJSUpdate() => match (true) {
                ...
                default => $this->router->redirect('post', [
                    'tid' => $topic->id,
                ]),
            },
    ...
    private function viewTopic(ModelsTopic $modelsTopic): void
    {
        ...
        $category =
            $forum !== null ? Category::selectOne($forum->category) : null;
        // Fix this to work with subforums.
        $this->page->setBreadCrumbs([
            $this->router->url('category', ['id' => $category?->id]) =>
                $category->title ?? '',
        ...
        $pageLinks = [];
        foreach (
            $this->jax->pages($totalpages, $this->pageNumber + 1, 10)
            as $pageNumber
        ) {
        ...
        $usersInTopic = array_filter(
            ...
            static fn(UserOnline $userOnline): bool => $userOnline->location ===
                "vt{$modelsTopic->id}",
        );
    ...
    private function renderBadges(array $posts): array
    {
        ...
        foreach ($badgesPerAuthor as $authorId => $badgeTuples) {
            ...
            foreach ($badgeTuples as $badgeTuple) {
                $profileBadgesURL = $this->router->url('profile', [
                    'id' => $authorId,
                    'page' => 'badges',
                ]);
    ...
    private function quickReplyForm(ModelsTopic $modelsTopic): void
    {
        ...
        if ($this->session->getVar('multiquote')) {
            ...
            foreach ($posts as $post) {
                $prefilled .=
                    '[quote=' .
                    $membersById[$post->author]->displayName .
                    ']' .
                    $post->post .
                    '[/quote]' .
                    PHP_EOL;
    ...
    private function viewRSS(ModelsTopic $modelsTopic): void
    {
        ...
        $rssFeed = new RSSFeed([
            ...
            'link' =>
                $boardURL .
                $this->router->url('topic', ['id' => $modelsTopic->id]),
            'title' => $this->textFormatting->wordFilter($modelsTopic->title),
        ]);
