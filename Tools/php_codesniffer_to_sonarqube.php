...
$issues = array_merge_recursive(
    ...array_map(static function (string $filename) use ($data): array {
        $file = new SplFileObject($filename);

        return array_map(static function (array $message) use (
            $file,
            $filename,
        ): array {
            $column = (static function (
                ...
                $tab_count = intdiv(
                    mb_substr_count($up_to_column, TAB_REPLACEMENT_CHARACTER),
                    TAB_WIDTH,
                );
