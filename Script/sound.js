var Sound = new function () {
  ...
  this.ready = function () {
    Sound.isready = true
    var q = Sound.queue
    Sound.flashObject = window.soundLoader && window.soundLoader.playSound ?
      window.soundLoader : document.soundLoader;
    if (!Sound.flashObject || !Sound.flashObject.loadSound) return alert(
      "Fucked up sounds");
    if (q.length)
      for (var x = 0; x < q.length; x++) Sound.load(q[x][0], q[x]
        [1], q[x][2] || false);
    Sound.queue = []
  }
