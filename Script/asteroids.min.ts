...
(function () {
    function Asteroids() {
        ...
        function addBlinkStyle() {
            addStylesheet(
                'ASTEROIDSYEAH',
                '.ASTEROIDSYEAHENEMY { outline: 2px dotted red; }',
            );
        }
        ...
        this.ctx.drawParticle = function (particle) {
            ...
            this.drawLine(
                particle.pos.x,
                particle.pos.y,
                particle.pos.x - particle.dir.x * 10,
                particle.pos.y - particle.dir.y * 10,
            );
        ...
        this.update = function () {
            ...
            if (
                this.keysPressed[code('right')] ||
                this.keysPressed[code('D')]
            ) {
                forceChange = true;
            ...
            for (var i = 0; i < this.bullets.length; i++) {
                ...
                var ray = new Line(
                    this.bullets[i].pos.cp(),
                    this.bullets[i].pos.addNew(bulletVel),
                );
                ...
                if (
                    murdered &&
                    indexOf(ignoredTypes, murdered.tagName.toUpperCase()) ==
                        -1 &&
            ...
            for (var i = 0, enemy; (enemy = this.dieing[i]); i++) {
                try {
                    if (enemy.parentNode) window.ASTEROIDS.enemiesKilled++;
            ...
            for (var i = 0; i < this.particles.length; i++) {
                this.particles[i].pos.add(
                    this.particles[i].dir.mulNew(
                        particleSpeed * tDelta * Math.random(),
                    ),
                );
