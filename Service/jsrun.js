...
function getJXBDBaseDir() {
  ...
  for (var i = 0; i < elts.length; i++) {
    var elt = elts[i];
    if (elt.src.substr(elt.src.length - 8, 8) == "jsrun.js") return elt.src
      .substr(0, elt.src.length - 8);
  }
  return null;
}

function IMWindow(uid, uname) {
  if (!globalsettings.can_im) return alert(
...
IMWindow.menu = function (e, uid) {
  ...
  document.body.onclick = function (e) {
    e = JAX.event(e);
    if (e.srcElement != d && !JAX.el.isChildOf(e.srcElement, d)) d.parentNode
      .removeChild(d);
  }
...
function RUNF() {
  ...
  this.updateDates = function () {
    ...
    for (x = 0; x < dates.length; x++) {
      parsed = JAX.el.hasClass(dates[x], 'smalldate') ? JAX.smalldate(
        parseInt(dates[x].title)) : JAX.date(parseInt(dates[x].title))
  ...
  stream.donext = function (a) {
    ...
    if (document.cookie.match("actw=" + window.name)) stream.timeout =
      setTimeout(stream.loader, updatetime);
  }
  stream.setup.callback = function (xmlobj) {
    ...
    if (xml.length) {
      ...
      for (x = 0; x < cmds.length; x++) {
        ...
        if (stream.commands[cmd]) var cmd = stream.commands[cmd](
          cmds[x])
      }
    }
    if (xmlobj.type >= 2) {
  ...
  stream.commands = {
    ...
    tick: function (a) {
      ...
      if (l > 100) {
        for (var x = 100; x < l; x++) {
          ...
          if (!tick.bonked) {
            tick = ticks[x]
            JAX.sfx(tick, 30, 500).add('opacity', '1', '0')
              .then(function (el) {
    ...
    im: function (a) {
      ...
      if (!window.hasFocus && window.webkitNotifications &&
      ...
      if (!sb) {
        ...
        win.title = a[1] +
          ' <a href="#" onclick="IMWindow.menu(event,' + a[0] +
          ');return false;">&rsaquo;</a>';
        win.content =
          "<div class='ims'></div><div class='offline'>This user may be offline</div><div><form onsubmit='return RUN.submitForm(this,1)' method='post'><input type='hidden' name='im_uid' value='%s' /><input type='text' name='im_im' /><input type='hidden' name='act' value='blank' /></form></div>"
          .replace(/%s/g, a[0])
        win.className = "im"
        win.resize = ".ims"
        win.animate = true
        ...
        if (a[2] && globalsettings.sound_im) Sound.play(
          "imnewwindow");
      }
      if (a[2]) {
        a[3] = parseInt(a[3])
        var d = document.createElement('div'),
          l, x, act = a[2].substring(0, 3) == "/me"
        if (act) {
          d.className = "action";
          a[2] = a[2].substring(3);
          a[1] = "***" + a[1];
        ...
        if (!a[3]) JAX.el.removeClass($('im_' + a[0]),
          'offline')
        d.innerHTML = "<a href='?act=vu" + (a[3] || parseInt(a[
          0])) + "' class='name'>" + a[1] + "</a> " + (!
          act ? ": " : "") + a[2]
        d.title = a[4]
        test = (sb.scrollTop > (sb.scrollHeight - sb.clientHeight) -
          50)
        sb.appendChild(d)
        if (test) sb.scrollTop = sb.scrollHeight
        JAX.sfx(d).dehighlight().play()
        JAX.gracefulDegrade(d)
        if (!win && globalsettings.sound_im) Sound.play(
          "imbeep")
      }
    },
    ...
    onlinelist: function (a) {
      ...
      for (x = 0; x < a[0].length; x++) {
        ...
        if (!link) {
          ...
          if (!isNaN(parseInt(tmp[0]))) link.href = "?act=vu" +
            tmp[0];
          link.innerHTML = tmp[3];
          link.onclick = RUN.stream.locationfunc
        }
        link.className = "user" + tmp[0] + " mgroup" + tmp[1] +
          " " + (tmp[2] ? " " + tmp[2] : "")
        if (tmp[4]) link.onmouseover = function () {
    ...
    scrollToPost: function (a) {
      var el = document.getElementById('pid_' + a[0]),
        pos
      if (!el) return false
      JAX.onImagesLoaded(document.getElementById('page').getElementsByTagName(
        'img'), function () {
    ...
    updateqreply: function (a) {
      if ($('qreply')) {
        ...
        $('qreply').getElementsByTagName('textarea')[0].value +=
          a[0]
      }
    },
    newmessage: function (a) {
      ...
      n.onclick = function () {
        ...
        RUN.stream.location('?act=ucp&what=inbox&view=' + a[
          1], 3);
      }
      n.innerHTML = a[0]
    },
    ...
    attachfiles: function () {
      ...
      u[1].addfile = function (file) {
        ...
        f.className = 'file';
        f.innerHTML = "<div class='name'>" + file.name +
      ...
      u[1].response = function (response) {
        $('pdedit').editor.cmd("inserthtml", "[attachment]" +
          response + "[/attachment]");
      }
    },
    listrating: function (a) {
      ...
      if (prdiv) {
        if (prdiv.style.display != 'none') {
          (new JAX.sfx(prdiv)).add('height', '200px', '0px').then(
            function () {
        ...
        c = JAX.el.getCoordinates($$('#pid_' + a[0] +
          ' .postrating'));
  ...
  while (foot && foot.tagName != "BODY" && (fs = JAX.el.getComputedStyle(foot)))
    if (fs.display == "none" || fs.visibility == "hidden") return alert(
      "WARNING: COPYRIGHT MISSING, THIS IS AGAINST TOS");
