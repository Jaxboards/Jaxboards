...
function RUNF(){
 ...
 this.submitForm=function(a,b){
  ...
  for(x=0;x<l;x++) {
   ...
   if(a[x].type=="select-multiple") {
    for(i=0;i<a[x].options.length;i++) if(a[x].options[i].selected) {names.push(a[x].name+"[]");values.push(a[x].options[i].value);}
    continue;
   }
 ...
 stream.donext=function(a){
  ...
  if(document.cookie.match("actw="+window.name)) stream.timeout=setTimeout(stream.loader,updatetime);
 }
 stream.setup.callback=function(xmlobj){
  if(xmlobj.status!=200) return;
  else xmlobj.parsed=true;
  var xml=xmlobj.responseText,db=$('debug'),softurl=false
  ...
  if(xml.length){
   ...
   for(x=0;x<cmds.length;x++) {
    cmd=cmds[x].shift()
    if(cmd=="softurl") {softurl=true;continue;}
 ...
 stream.locationfunc=function(){void(0);RUN.stream.location(this.getAttribute('href'));return false;}
 ...
 stream.updatePage=function(){
  ...
  var l
  if((l=(document.location.hash.substring(1)||""))!=(stream.lastURL)) {stream.location(l,"3")}
 }
 stream.donext()
 ...
 // Commands
 stream.commands={
  script:function(a){eval(a[0]);},
  error:function(a){alert(a[0]);},
  alert:function(a){alert(a[0]);},
  addclass:function(a){JAX.el.addClass($$(a[0]),a[1]);},
  title:function(a){document.title=a;},
  update:function(a){
        var el=a[0],paths
        if(el=="path"&&(paths=$$('.path')).length>1) {
         for(var x=0;x<paths.length;x++){paths[x].innerHTML=a[1];JAX.gracefulDegrade(paths[x]);}
         return;
        }
        if(!el.match(/^\W/)) el="#"+el
        el=$$(el)
        if(!el) return
	el.innerHTML=a[1];
        if(a[2]) JAX.sfx(el).dehighlight().play()
  ...
  setloc:function(a){document.location="#"+a;stream.lastURL="?"+a;},
  setstatus:function(a){if($('status')) $('status').className=a[0];},
  ...
  location:function(a){if(a[0].charAt(0)=='?') RUN.stream.location(a[0]);else document.location=a[0];},
  enable:function(a){a=$(a[0]);if(a) a.disabled=false;},
  addshout:function(a){
   var a=a[0],ss=$$('#shoutbox .shout'),x,
   span=document.createElement('span'),div;
   span.innerHTML=a;
   if(ss&&!ss.length) ss=[ss]
   div=span.firstChild;
   ss[0].parentNode.insertBefore(div,ss[0]);
   while(ss.length>globalsettings.shoutlimit-1) {x=ss.pop();x.parentNode.removeChild(x);}
   JAX.sfx(div).dehighlight().play()
  ...
  tick:function(a){
   ...
   tick.className='tick'
   tick.innerHTML=a[0]
   tick.style.display='none';tick.style.overflow='hidden';
   ticker.insertBefore(tick,ticker.firstChild)
   var h=JAX.el.getComputedStyle(tick);h=h.height;
   tick.style.height='0px';
   JAX.sfx(tick).add('height','0px',h).play();
   var ticks=$$('.tick',ticker),l=ticks.length
   ...
   if(l>100) {
    for(var x=100;x<l;x++) {
     ...
     if(!tick.bonked) {
      tick=ticks[x]
      JAX.sfx(tick,30,500).add('opacity','1','0').then(function(el){el.parentNode.removeChild(el);}).play()
      tick.bonked=true
     }
    }
   }
  ...
  im:function(a){
   var sb=$$('#im_'+a[0]+' .ims'),test
   JAX.flashTitle("New message from "+a[1]+"!")
   if(!window.hasFocus&&window.webkitNotifications&&webkitNotifications.checkPermission()==0) {
    var notify=webkitNotifications.createNotification('',a[1]+" says:",a[2])
    notify.show()
    notify.onclick=function(){window.focus();notify.cancel();}
   }
   if(!sb) {
    var win=new JAX.window
    win.title=a[1]+' <a href="#" onclick="IMWindow.menu(event,'+a[0]+');return false;">&rsaquo;</a>';
    win.content="<div class='ims'></div><div class='offline'>This user may be offline</div><div><form onsubmit='return RUN.submitForm(this,1)' method='post'><input type='hidden' name='im_uid' value='%s' /><input type='text' name='im_im' /><input type='hidden' name='act' value='blank' /></form></div>".replace(/%s/g,a[0])
    win.className="im"
    win.resize=".ims"
    win.animate=true
    win=win.create()
    ...
    win.id="im_"+a[0]
    win.onclick=function(){win.getElementsByTagName('form')[0].im_im.focus();}
    win.onclick()
    sb=$$('#im_'+a[0]+' .ims')
    test=JAX.el.getComputedStyle(sb)
    sb.style.width=test.width;sb.style.height=test.height;
    if(a[2]&&globalsettings.sound_im) Sound.play("imnewwindow");
   }
   if(a[2]) {
    a[3]=parseInt(a[3])
    var d=document.createElement('div'),l,x,act=a[2].substring(0,3)=="/me"
    if(act) {d.className="action";a[2]=a[2].substring(3);a[1]="***"+a[1];}
	JAX.el.addClass(d,a[3]?"you":"them")
	if(!a[3]) JAX.el.removeClass($('im_'+a[0]),'offline')
    d.innerHTML="<a href='?act=vu"+(a[3]||parseInt(a[0]))+"' class='name'>"+a[1]+"</a> "+(!act?": ":"")+a[2]
    d.title=a[4]
    test=(sb.scrollTop>(sb.scrollHeight-sb.clientHeight)-50)
    sb.appendChild(d)
    if(test) sb.scrollTop=sb.scrollHeight
    JAX.sfx(d).dehighlight().play()
  ...
  openbuddylist:function(a){
   ...
   if(!buddylist) {
    ...
    win.animate=0
    win.wait=false
    win.onclose=function(){document.cookie='buddylist=0';}
    win.resize=".content"
    win=win.create()
   } else {
  ...
  onlinelist:function(a){
   var html="",tmp,link,statusers=$('statusers'),newlink
   if(!statusers) return
   for(x=0;x<a[0].length;x++) {
    tmp=a[0][x]
    link=$$('#statusers .user'+tmp[0])
    if(!link) {link=document.createElement('a');if(!isNaN(parseInt(tmp[0]))) link.href="?act=vu"+tmp[0];link.innerHTML=tmp[3];link.onclick=RUN.stream.locationfunc}
    link.className="user"+tmp[0]+" mgroup"+tmp[1]+" "+(tmp[2]?" "+tmp[2]:"")
    if(tmp[4]) link.onmouseover=function(){JAX.tooltip(this,this.title);}
    link.title=tmp[4]
    if(tmp[2]!="idle") {
     if(statusers.firstChild) {statusers.insertBefore(link,statusers.firstChild);}
     else statusers.appendChild(link)
    }
   }
  },
  setoffline:function(a){
   var statusers=$('statusers'),ids=a[0].split(","),x,link
  ...
  scrollToPost:function(a){
    var el=document.getElementById('pid_'+a[0]),pos
    if(!el) return false
    JAX.onImagesLoaded(document.getElementById('page').getElementsByTagName('img'),function(){
  ...
  newmessage:function(a){
   var n=$('notification'),num=$('num-messages')
   ...
   n.onclick=function(){n.style.display='none';RUN.stream.location('?act=ucp&what=inbox&view='+a[1],3);}
   n.innerHTML=a[0]
  },
  setsound:function(a){Sound.load(a[0],a[1],false);},
  playsound:function(a){Sound.loadAndPlay(a[0],a[1],a[2]?true:false);},
  attachfiles:function(){
   var el=$('attachfiles');
   var u=Uploader.createButton()
   var d=document.createElement('div');d.className='files';d.appendChild(u[0]);
   JAX.el.replace(el,d)
   u[1].addfile=function(file){if(file.size>5242880) {setTimeout(function(){alert("Files can't be over 5MB");},1000);return;}var f=document.createElement('div');f.className='file';f.innerHTML="<div class='name'>"+file.name+"</div><div class='progressbar'><div class='progress' id='progress_"+this.id+'_'+file.id+"' style='width:0px'></div></div>";d.appendChild(f);file.upload("/index.php?act=post&uploadflash=1&sessid="+document.cookie.match('sid=([^;]+)')[1]);}
   u[1].error=function(error,shit){var w=new JAX.window;w.title="error";w.content=shit;w.create();}
   u[1].progress=function(file,b,c){$('progress_'+this.id+'_'+file.id).style.width=Math.round(b/c*100)+"%";}
   u[1].response=function(response){$('pdedit').editor.cmd("inserthtml","[attachment]"+response+"[/attachment]");}
  },
  listrating:function(a){
   var prdiv=$('postrating_'+a[0]),c
   if(prdiv) {
    if(prdiv.style.display!='none') {
     (new JAX.sfx(prdiv)).add('height','200px','0px').then(function(){prdiv.style.display='none';}).play()
     return
...
$(function(){RUN=new RUNF})
$(Sound.addFlash)
$(function(){window.loaded=true;window.name=Math.random();window.hasFocus=true;addEvent(window,"onblur",function(){window.hasFocus=false;});addEvent(window,"onfocus",function(){RUN.setWindowActive();});})
