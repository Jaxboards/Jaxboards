...
function addEvent(obj, att, set) {
  ...
  if (!obj[atts]) {
    ...
    obj[att] = function (e) {
      for (var x = 0; x < obj[atts].length; x++) obj[atts][x].call(
        this, e);
    }
  }
  obj[atts].push(set)
...
function $$(s, base) {
  ...
  for (var y = 0; y < l; y++) {
    ...
    case ".":
      ...
      curel = []
      if (curel2.length)
        for (x = 0; x < curel2.length; x++) curel = curel.concat(JAX.el
          .getElementsByClassName(curel2[x] || base, RegExp.$2))
    ...
    default:
      ...
      curel = []
      if (curel2.length)
        for (x = 0; x < curel2.length; x++) curel = curel.concat(curel2[
          x].getElementsByTagName(RegExp.$2))
...
var JAX = new function () {
  ...
  this.date = function (a) {
    var old = new Date,
      now = new Date,
      fmt, hours, mins, delta, ampm, yday = new Date,
      dstr,
    ...
    else if (delta < 3600) fmt = Math.round(delta / 60) +
      " minutes ago";
    else if (now.getDate() + ' ' + now.getMonth() + ' ' + now.getFullYear() ==
      dstr) fmt = "Today @ " + hours + ':' + JAX.prepad(mins, 2) +
      ' ' + ampm;
    else if (yday.getDate() + ' ' + yday.getMonth() + ' ' + yday.getFullYear() ==
      dstr) fmt = "Yesterday @ " + hours + ':' + JAX.prepad(mins,
      2) + ' ' + ampm;
    else fmt = months[old.getMonth()] + " " + JAX.ordsuffix(old.getDate()) +
      ", " + old.getFullYear() + " @ " + hours + ':' + JAX.prepad(
        mins, 2) + ' ' + ampm;
    return fmt;
  }
  ...
  this.smalldate = function (a) {
    ...
    h %= 12
    h = h || 12
    return h + ':' + JAX.prepad(d.getMinutes(), 2) + ampm + ', ' +
      (d.getMonth() + 1) + '/' + JAX.prepad(d.getDate(), 2) + '/' +
      d.getFullYear()
  }

  this.ordsuffix = function (a) {
    return a + (Math.round(a / 10) == 1 ? 'th' : ['', 'st', 'nd',
      'rd'
    ][a % 10] || 'th');
  }
  ...
  this.toJSON = function (shit) {
    ...
    } else if (typeof shit == "string") return '"' + shit.replace(
      /"/g, '\\"') + '"';
    else return shit;
  ...
  this.autoComplete = function (a, el, dummy, e) {
    ...
    if (e.UP && l && sindex >= 1 && typeof sindex != 'undefined') {
    ...
    } else if (e.DOWN && l && (sindex < l - 1 || typeof sindex ==
        'undefined')) {
      ...
      var a = (new JAX.ajax).load((document.location.toString().match(
          '/acp/') ? '../' : '') + 'misc/listloader.php?' +
        a,
        function (xml) {
          var x, tmp
  ...
  this.flashTitle = function (a) {
    ...
    JAX.flashInterval = setInterval(function () {
      document.title = (document.title == document.titleorig ?
        document.title2 : document.titleorig);
    }, 1000);
  }
  ...
  this.gracefulDegrade = function (a) {
    ...
    var tmp, links = a.getElementsByTagName('a'),
      l = links.length,
      x, old
        ...
        if (links[x].getAttribute('href').charAt(0) == "?") {
          ...
          links[x].onclick = function () {
            if (!this.oldclick || this.oldclick() !=
              false) RUN.stream.location(this.getAttribute(
              'href'));
            return false;
          }
        } else if (links[x].getAttribute('href').substr(0, 4) ==
          "http") {
    ...
    if (bbcodeimgs) JAX.onImagesLoaded(bbcodeimgs, function () {
      ...
      if (galleries && !galleries.length) galleries = [
        galleries
      ]
      for (x = 0; x < galleries.length; x++) JAX.makeImageGallery(
        galleries[x])
    }, 2000)

    if (tmp = $$('.pages', a)) {
      if (!tmp.length) tmp = [tmp]
      for (x = 0; x < tmp.length; x++) JAX.scrollablepagelist(tmp[
        x])
    }
  ...
  this.convertSwitches = function (switches) {
    ...
    var x, l = switches.length,
      s, t
    for (x = 0; x < l; x++) {
      s = switches[x]
      t = document.createElement('div')
      t.className = s.className.replace("switch",
        "switch_converted")
      t.s = s
      ...
      t.onclick = function () {
        ...
        this.style.backgroundPosition = this.s.checked ?
          'top' : 'bottom';
  ...
  this.checkAll = function (checkboxes, value) {
    for (var x = 0; x < checkboxes.length; x++) checkboxes[x].checked =
      value
  }
  ...
  this.collapse = function (a) {
    ...
    s.overflow = 'hidden'
    if (s.height == '0px') {
      JAX.sfx(a, 5, 10, 0).add('height', '0px', fh).then(function () {
      ...
      JAX.sfx(a, 5, 10, 0).add('height', fh, '0px').then(function () {
  ...
  this.el = {
    apply: function (a) {
      var func
      for (var x in JAX.el) {
        if (x != "apply") a[x] = new Function("b",
          "return JAX.el." + x + "(this,b)");
      }
      return a
    },
    getComputedStyle: function (a, b) {
      ...
      else if (window.getComputedStyle) return window.getComputedStyle(
        a, b)
      return false
    },
    ...
    getElementsByClassName: function (parent, classn) {
      ...
      var els = parent.getElementsByTagName("*"),
        x, r = [],
        regex = new RegExp(" " + classn + "| " + classn +
          " |" + classn + " |^" + classn + "$")
    ...
    getHighestZIndex: function () {
      var a = document.getElementsByTagName("*"),
        ...
        x, max = 0
      for (x = 0; x < l; x++)
        if (a[x].style.zIndex && Number(a[x].style.zIndex) >
          max) max = Number(a[x].style.zIndex)
      return max + 1
    }
  }
  /********************************************************************/
  ...
  this.color = function (a) {
      ...
      if (typeof a == "object") this.rgb = a
        ...
        if (a.length == 3) a = a.charAt(0) + a.charAt(0) + a.charAt(
          1) + a.charAt(1) + a.charAt(2) + a.charAt(2)
        if (a.length != 6) this.rgb = [0, 0, 0]
        else {
          this.rgb = [];
          for (x = 0; x < 3; x++) this.rgb[x] = parseInt(a.substr(
            x * 2, 2), 16);
        }
      } else this.rgb = [0, 0, 0]

      this.invert = function () {
        this.rgb = [255 - this.rgb[0], 255 - this.rgb[1], 255 -
          this.rgb[2]
        ];
        return this;
      }
      ...
      this.toHex = function () {
        ...
        for (x = 0; x < 3; x++) {
          tmp2 = this.rgb[x]
          tmp += hex.charAt(Math.floor(tmp2 / 16)) + hex.charAt(
            Math.floor(tmp2 % 16))
        }
        return tmp
      }
    ...
    this.sfx = function (el, steps, delay, loop) {
      ...
      me.morph = function (from, percent, to) {
        ...
        if (JAX.isArray(from) && from.length == to.length) {
          r = [];
          for (x = 0; x < from.length; x++) r[x] = Math.round(
            this.morph(from[x], percent, to[x]));
        } else {
      ...
      me.step = function () {
        ...
        if (typeof curL[0] == "function") {
          ...
          for (x = 0; x < curL.length; x++) {
            ...
            } else if (tmp[0] != "opacity") tmp2 = Math.round(
              tmp2)
      ...
      me.add = function (what, from, to) {
        ...
        me.lineup[me.lineup.length - 1].push([what, from, t[1],
          t[0], t[2]
        ]);
        return this
      }
      me.dehighlight = function () {
        ...
        var bg = JAX.el.getComputedStyle(me.el).backgroundColor
          .toString(),
          bg2
  ...
  this.editor = function (textarea, iframe) {
    ...
    me.mode = (JAX.browser.mobile || JAX.browser.n3ds ? 0 :
      globalsettings.wysiwyg) //1 for html editing mode, 0 for textarea mode
    me.mode = me.mode || 0
    ...
    me.buildEditBar = function () {
      ...
      var cmds = ['bold', 'italic', 'underline',
          'strikethrough', 'forecolor', 'backcolor',
          'insertimage', 'createlink',
          'c_email', 'justifyleft', 'justifycenter',
          'justifyright', 'c_youtube',
          'c_code', 'c_quote', 'c_spoiler',
          'insertorderedlist', 'insertunorderedlist',
          'c_smileys', 'c_switcheditmode'
        ],
        cmddesc = ['Bold', 'Italic', 'Underline',
          'Strike-Through', 'Foreground Color',
          'Background Color', 'Insert Image',
          'Insert Link',
          'Insert email', 'Align left', 'Center',
          'Align right',
          'Insert video from any of your favorite video services!',
          'Insert code', 'Insert Quote', 'Insert Spoiler',
          'Create Ordered List', 'Create Unordered List',
    ...
    me.editbarCommand = function (e, cmd) {
      ...
      if (cmd == "forecolor" || cmd == "backcolor") me.showColors(
        e.pageX, e.pageY, cmd)
      else if (cmd == "c_smileys") me.showEmotes(e.pageX, e.pageY)
      else if (cmd == "c_switcheditmode") me.switchMode(Math.abs(
        me.mode - 1))
      else this.cmd(cmd)
    }
    ...
    me.hideEmotes = function () {
      if (me.emoteWindow) me.emoteWindow.style.display =
        'none'
    }

    me.createEmoteWindow = function (xml) {
      var rs = eval(xml.responseText),
        x, html, emotewin = document.createElement('div'),
        r, t
      ...
      me.showEmotes(me.createEmoteWindow.x, me.createEmoteWindow
        .y)
    }
    ...
    me.showColors = function (posx, posy, cmd) {
      if (me.colorWindow && me.colorWindow.style.display !=
        'none') return me.hideColors();
      var colorwin = me.colorWindow,
        colors = ['FFFFFF', 'AAAAAA', '000000', 'FF0000',
    ...
    me.cmd = function (a, b, c) {
      a = a.toLowerCase()
      var rng, selection = me.getSelection(),
        bbcode
      switch (a) {
      case "bold":
      ...
      case "c_email":
        ...
        break;
      case "backcolor":
        if (JAX.browser.firefox || JAX.browser.safari) a =
          "hilitecolor";
        //a="inserthtml";b='<span style="background:'+b+'">'+selection+'</span>'
        bbcode = "[bgcolor=" + b + "]" + selection +
          "[/bgcolor]"
        break;
      case "forecolor":
        bbcode = "[color=" + b + "]" + selection +
          "[/color]"
        break;
      case "c_code":
        a = "inserthtml";
      ...
      if (me.mode) {
        if (a == "inserthtml" && JAX.browser.ie) {
          ...
          if (!rng.text.length) me.doc.body.innerHTML +=
            b
          else {
            rng.pasteHTML(b)
            rng.collapse(false)
          ...
          if (me.iframe.contentWindow.focus) me.iframe.contentWindow
            .focus()
        }
    ...
    me.getSelection = function () {
      if (me.mode) return JAX.browser.ie ? me.doc.selection.createRange()
        .text : me.window.getSelection();
      else {
        if (JAX.browser.ie) {
          me.textarea.focus()
          return document.selection.createRange().text
        } else {
          return me.textarea.value.substring(me.textarea.selectionStart,
    ...
    me.HTMLtoBB = function (a) {
      ...
      a = a.replace(/<img.*?src=["']?([^'"]+)["'][^>]*\/?>/g,
        "[img]$1[/img]");
      a = me.replaceAll(a, /<(\w+)([^>]*)>([\w\W]*?)<\/\1>/ig,
        function (whole, tag, attributes, innerhtml) {
          var att = {},
            style = ''
          attributes.replace(
          ...
          if (style.match(
            ...
            )) innerhtml = "[bgcolor=#" + (new JAX.color(
              RegExp.$2)).toHex() + "]" +
            innerhtml + "[/bgcolor]"
          if (style.match(
              /text\-align: ?(right|center|left);/i
            )) innerhtml = "[align=" + RegExp.$1 +
            "]" + innerhtml + "[/align]"
          if (style.match(/font\-style: ?italic;/i) ||
            tag == "i" || tag == "em") innerhtml =
            "[I]" + innerhtml + "[/I]"
          if (style.match(
          ...
          if (style.match(
            ...
            innerhtml + "[/S]"
          if (style.match(/font\-weight: ?bold;/i) ||
            tag == "strong" || tag == "b") innerhtml =
            "[B]" + innerhtml + "[/B]"
          if (att.size || style.match(
              /font\-size: ?([^;]+)/i)) innerhtml =
            '[size=' + (att.size || RegExp.$1) +
            ']' + innerhtml + '[/size]'
          if (att.color || style.match(
              /color: ?([^;]+)/i)) innerhtml =
            "[color=" + (att.color || RegExp.$1) +
            ']' + innerhtml + '[/color]'
          if (tag == "a" && att.href) innerhtml =
            '[url=' + att.href + ']' + innerhtml +
            '[/url]'
          if (tag == "ol") innerhtml = "[ol]" +
            innerhtml + "[/ol]"
          if (tag == "ul") innerhtml = "[ul]" +
            innerhtml + "[/ul]"
          if (tag.match(/h\d/i)) innerhtml = "[" +
            tag.toLowerCase() + "]" + innerhtml +
            "[/" + tag.toLowerCase() + "]"
          if (tag == "li") innerhtml = '*' +
            innerhtml.replace(/[\n\r]+/, '') + "\n"
          if (tag == "p") innerhtml = "\n" + (
  ...
  this.editor.setSelection = function (t, stuff) {
    ...
    if (JAX.browser.ie) {
      ...
      var s = t.selectionStart,
        e = t.selectionEnd
      t.value = t.value.substring(0, s) + stuff + t.value.substr(
        e)
  ...
  this.drag = function () {
    ...
    me.priv = {
      start: function (e, t, handle) {
        ...
        if (me._nochild && (e.srcElement || e.target) !=
          (handle || el)) return;
        ...
        if (!me.sess.zIndex || Number(me.sess.zIndex) <
          (highz - 1)) el.style.zIndex = highz
        if (typeof me.onstart == 'function') {
          var sess = me.sess
          sess.droptarget = me.priv.testdrops(sess.mx,
            sess.my);
          me.onstart(sess)
        }
      ...
      drag: function (e) {
        ...
        var s = me.sess.el.style,
          sess, tmp = false,
          tx, ty, tmp2
        var tx, ty, mx = tx = parseInt(e.pageX),
        ...
        if (b) {
          if (left < b[0]) {
            mx = mx - left + b[0];
            left = b[0];
          } else if (left > b[0] + b[2]) left = b[0] +
            b[2];
          if (top < b[1]) {
            my = my - top + b[1];
            top = b[1];
          } else if (top > b[1] + b[3]) top = b[1] +
            b[3];
        }
        s.left = left + "px";
        s.top = top + "px";
        ...
        if (typeof me.ondrag == 'function') me.ondrag(
          sess)
        if (typeof me.ondragover == 'function' && sess[
            'droptarget'] && tmp != sess[
            'droptarget']) me.ondragover(sess)
        if (typeof me.ondragout == 'function' && tmp &&
          sess['droptarget'] != tmp) {
      ...
      drop: function () {
        document.onmousemove = document.onmouseup =
          function () {}
        if (typeof me.ondrop == 'function') me.ondrop(
          me.sess.info)
        if (!me._autoz) me.sess.el.style.zIndex = me.sess
          .zIndex;
        return true
      },
      testdrops: function (a, b) {
        var x, d = me.droppables,
          z, r = false,
          max = [9999, 9999]
        if (!d) return r
        for (x = 0; x < d.length; x++) {
          if (d[x] == me.sess.el || JAX.el.isChildOf(
              d[x], me.sess.el)) continue;
          z = JAX.el.getCoordinates(d[x]);
          if (max[0] > z.w && max[1] > z.h && a >= z.x &&
            b >= z.y && a <= z.xw && b <= z.yh) {
            max = [z.w, z.h];
            r = d[x];
          }
    ...
    me.apply = function (el, t) {
      ...
      pos = pos.position;
      if (!pos || pos == 'static') el.style.position =
        'relative';
      (t || el).onmousedown = t ? function (e) {
  ...
  this.sortable = function (a, b) {
    ...
    me.ondrag = function (a) {
      var x, y, d = me.elems,
        dl = d.length,
        pos = 0,
        c, cel = JAX.el.getCoordinates(a.el),
        c2, ch = false,
        ov = me.options.vertical || 0,
        oh = me.options.horizontal || 0,
        index
      if (!me.coords.length) {
        for (x = 0; x < dl; x++) me.coords.push(JAX.el.getCoordinates(
          d[x]));
      }
      for (x = 0; x < dl; x++) {
        if (a.el == d[x]) {
        ...
        c = me.coords[x]
        if (ch === false && (ov ? a.my < c.yh && a.dy < 0 :
            a.mx < c.xw && a.my < c.yh)) {
      ...
      if (ch === false) {
        for (x = dl - 1; x >= index; x--) {
          ...
          c = me.coords[x]
          if ((ov ? a.my > c.y && a.dy > 0 : a.mx > c.x &&
              a.my > c.y)) {
    ...
    for (var x = 0; x < a.length; x++) {
      me.apply(a[x], typeof b.handle == 'function' ? b.handle(a[x]) :
        null)
    }
  }
  /*******************************************************************/
  this.sortableTree = function (tree, prefix, formfield) {
    ...
    function parsetree(tree) {
      ...
      var order = {},
        node, sub, gotsomethin = 0
      for (var x = 0; x < nodes.length; x++) {
        node = nodes[x]
        if (node.className != "seperator" && node.parentNode ==
          tree) {
          gotsomethin = 1
          sub = node.getElementsByTagName('ul')[0]
          order['_' + node.id.substr(prefix.length)] = sub !=
            undefined ? parsetree(sub) : 1
        }
      }
    ...
    drag.drops(seperators.concat(items)).addListener({
      ...
      ondrop: function (a) {
        var next = a.droptarget.nextSibling,
          tmp, tmp2
        var parentlock = (a.el.className ==
          'parentlock')
        var nofirstlevel = (a.el.className ==
          'nofirstlevel')
        if (a.droptarget) a.droptarget.style.border =
          'none';
        if (a.droptarget.className == 'seperator') {
          if (parentlock && a.droptarget.parentNode !=
            a.el.parentNode) return drag.reset(
            a.el, 1);
          if (nofirstlevel && a.droptarget.parentNode
            .className == 'tree') return drag.reset(
            a.el, 1);
          if (JAX.el.isChildOf(a.droptarget, a.el) ||
            a.el == next) return drag.reset(a.el,
            1);
          if (next.className == "spacer") {
            next.parentNode.removeChild(next);
          }
          if (next.className != "spacer") JAX.el.insertAfter(
            a.el.previousSibling, a.droptarget
          );
          else a.el.previousSibling.parentNode.removeChild(
            a.el.previousSibling)
          JAX.el.insertAfter(a.el, a.droptarget);
        } else if (!parentlock && a.droptarget.tagName ==
          "LI") {
          tmp = a.droptarget.getElementsByTagName(
            'ul')[0]
          if (!tmp) {
        ...
        drag.reset(a.el, 1)
        if (formfield) formfield.value = JAX.toJSON(
          parsetree(tree))
      }
    });
  ...
  this.event = function (e) {
    ...
    if (typeof e.pageY == 'undefined') {
      e.pageY = e.clientY + (parseInt(dE.scrollTop || dB.scrollTop) ||
        0);
      e.pageX = e.clientX + (parseInt(dE.scrollLeft || dB.scrollLeft) ||
        0);
    }
    e.cancel = function () {
      e.returnValue = false;
  ...
  this.ajax = function (s) {
    var me = this
    me.xmlobj = window.XMLHttpRequest ? XMLHttpRequest :
    ...
    me.load = function (a, b, c, d, e) {
      ...
      if (d) d = "POST"
      if (c && JAX.isArray(c) && JAX.isArray(c[0]) && c[0].length ==
        c[1].length) c = me.build_query(c[0], c[1])
      ...
      xmlobj.url = a
      xmlobj.type = e
      if (d) xmlobj.setRequestHeader("Content-Type",
  ...
  this.ajax.prototype.build_query = function (a, b) {
    var q = ""
    if (b)
      for (x = 0; x < a.length; x++) q += encodeURIComponent(a[x]) +
        "=" + encodeURIComponent(b[x] || "") + "&";
    else
      for (x in a) q += encodeURIComponent(x) + "=" +
        encodeURIComponent(a[x] || "") + "&";
  ...
  this.imageResizer = function (imgs) {
    ...
    for (var x = 0; x < imgs.length; x++) {
      p = p2 = 1
      img = imgs[x], nw = iw = parseInt(img.naturalWidth), nh =
        ih = parseInt(img.naturalHeight)
  ...
  this.makeResizer = function (iw, nw, ih, nh, img) {
    ...
    c.onmouseout = function () {
      if (this.scrollLeft) this.scrollLeft = this.scrollTop =
        0;
  ...
  this.window = function () {
    ...
    me.create = function () {
      ...
      if (me.resize) {
        ...
        (new JAX.drag).boundingBox(100, 100, Infinity,
          Infinity).addListener({
          ondrag: function (a) {
            var w = parseFloat(targ.style.width) +
              a.dx,
              h = parseFloat(targ.style.height) +
              a.dy
            targ.style.width = w + "px";
            if (w < d1.clientWidth - 20) {
              targ.style.width = d1.clientWidth +
                'px';
            } else {
              rsize.style.left = (d1.clientWidth -
                16) + "px";
            }
      ...
      if (me.wait) JAX.onImagesLoaded(d1.getElementsByTagName(
        'img'), function () {
        me.setPosition(pos)
      }, 2000);
      else me.setPosition(pos)

      me.drag = (new JAX.drag).autoZ().noChildActivation().boundingBox(
        0, 0, dE.clientWidth - 50, dE.clientHeight - 50
      ).apply(d1, d2);
      d1.close = me.close
    ...
    me.close = function () {
      ...
      if (me.animate && false) {
        ...
        JAX.sfx(me.open, 10)
          .add("top", s.top, (parseFloat(s.top) + 100) +
            "px")
          .then(function () {
    ...
    me.minimize = function () {
      ...
      if (JAX.el.hasClass(c, "minimized")) {
        ...
        for (x = 0; x < wins.length; x++)
          if (JAX.el.hasClass(wins[x], 'minimized')) w +=
            parseInt(wins[x].clientWidth)
    ...
    me.setPosition = function (pos, animate) {
      ...
      if (me.animate && animate !== 0) {
        JAX.sfx(d1, 10).add("top", (y - 100) + "px", y +
          "px").play()
  ...
  this.scrollablepagelist = function (pl) {
    if (pl.addEventListener) pl.addEventListener('DOMMouseScroll',
      scrollpagelist, false)
      ...
      status = {
        ...
        update: function () {
          ...
          for (x = 0; x < imgs.length; x++) {
            ...
            img.style.display = (x != this.index) ?
              'none' : 'block'
          }
        }
      }
  ...
  this.datepicker = new function () {
    ...
    this.daysshort = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri",
      "Sat"
    ] //I don't think I'll need a dayslong ever

    this.init = function (el) {
      var dp = $('datepicker'),
        s, c = JAX.el.getCoordinates(el),
        x
      if (!dp) {
      ...
      s = el.value.split('/')
      if (s.length == 3) {
        this.selectedDate = [parseInt(s[2]), parseInt(s[0]) -
          1, parseInt(s[1])
        ];
      } else this.selectedDate = undefined;

      this.el = el
      this.generate(s[2], s[0] ? parseInt(s[0]) - 1 :
        undefined, s[1])
    }
    ...
    this.generate = function (year, month, day) {
      ...
      for (x = 0; x < 7; x++) row.insertCell(x).innerHTML =
        this.daysshort[x]
      ...
      for (x = 0; x < numdaysinmonth; x++) {
        if (!x)
          for (i = 0; i < first; i++) row.insertCell(i)
        if ((first + x) % 7 == 0) row = dp.insertRow(dp.rows
          .length)
        cell = row.insertCell((first + x) % 7)
        cell.onclick = function () {
          JAX.datepicker.insert(this);
        }
        cell.className = 'day' + (year == this.selectedDate[
            0] && month == this.selectedDate[1] &&
    ...
    this.insert = function (cell) {
      var l = this.lastDate
      this.el.value = (l[1] + 1) + "/" + (cell.innerHTML) +
        "/" + l[0]
      this.hide()
    }
    this.hide = function () {
...
var Uploader = new function () {
  ...
  this.listenerHandler = function (id, action, args) {
    ...
    if (this.uploaders[id] && this.uploaders[id][action]) this.uploaders[
      id][action].apply(this.uploaders[id], args)
  }
...
var Sound = new function () {
  ...
  this.ready = function () {
    ...
    setTimeout(function () {
      ...
      var q = me.queue
      me.flashObject = window.soundLoader && window.soundLoader
        .playSound ? window.soundLoader : document.soundLoader;
      if (!me.flashObject || !me.flashObject.loadSound)
        return; //no flash installed
      if (q.length)
        for (var x = 0; x < q.length; x++) me.flashObject
          .loadSound(q[x][0], q[x][1], q[x][2] ||
            false);
      me.queue = []
      me.isready = true
    }, 100)
  ...
  this.load = function (title, file, autoplay) {
    ...
    if (this.isready) this.flashObject.loadSound(title, file,
      autoplay || false)
  ...
  this.addFlash = function () {
    document.body.appendChild(JAX.SWF("Script/soundLoader.swf",
      "soundLoader", {
        hidden: 'true',
        allowScriptAccess: 'sameDomain',
        width: 0,
