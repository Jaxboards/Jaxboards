...
String.prototype.striphtml=function(){return this.valueOf().replace(/</g,"&lt;").replace(/>/g,"&gt;");}

Math.rand=function(a,b){if(!b) {b=a;a=0;} return Math.round(Math.random()*b+a);}
...
if(!obj[atts]) {
 ...
 obj[att]=function(e){for(var x=0;x<obj[atts].length;x++)obj[atts][x].call(this,e);}
}
obj[atts].push(set)
...
function $$(s,base){
 var x,y,s=s.split(' '),l=s.length,curel=[],base=base||document
 for(var y=0;y<l;y++) {
  s[y].match(/([\#.]?)(.+)/)
  switch(RegExp.$1){
   case "#":
   ...
   case ".":
    ...
    curel=[]
    if (curel2.length)
     for(x=0;x<curel2.length;x++) curel=curel.concat(JAX.el.getElementsByClassName(curel2[x]||base,RegExp.$2))
   ...
   default:
    ...
    curel=[]
    if (curel2.length)
     for(x=0;x<curel2.length;x++) curel=curel.concat(curel2[x].getElementsByTagName(RegExp.$2))
...
var JAX=new function(){
 ...
 this.date=function(a){
  ...
  else if (now.getDate()+' '+now.getMonth()+' '+now.getFullYear()==dstr)  fmt="Today @ "+hours+':'+JAX.prepad(mins,2)+' '+ampm;
  else if (yday.getDate()+' '+yday.getMonth()+' '+yday.getFullYear()==dstr) fmt="Yesterday @ "+hours+':'+JAX.prepad(mins,2)+' '+ampm;
 ...
 this.smalldate=function(a){
  ...
  h%=12
  h=h||12
  return h+':'+JAX.prepad(d.getMinutes(),2)+ampm+', '+(d.getMonth()+1)+'/'+JAX.prepad(d.getDate(),2)+'/'+d.getFullYear()
 }
 ...
 this.toJSON=function(shit){
  var x,r="",ar=JAX.isArray(shit)
  if(ar){
   for(x=0;x<shit.length;x++){
    r+=JAX.toJSON(shit[x])+',';
   }
  } else if(typeof shit=="object") {
   for(x in shit){
    r+='"'+x+'":'+JAX.toJSON(shit[x])+',';
   }
  } else if(typeof shit=="string") return '"'+shit.replace(/"/g,'\\"')+'"';
  else return shit;
 ...
 this.autoComplete=function(a,el,dummy,e){
  ...
  el.onkeydown=function(e){e=JAX.event(e);if(e.ENTER) {e.cancel();return false;}}
  var d=$('autocomplete'),coords=JAX.el.getCoordinates(el),els,sindex,x,l=0
  if(!d) {
   ...
   l=els.length||0
   for(x=0;x<l;x++) if(JAX.el.hasClass(els[x],'selected')) {sindex=x;break;}
  }
  ...
  if(e.UP&&l&&sindex>=1&&typeof sindex!='undefined'){
  ...
  } else if (e.DOWN&&l&&(sindex<l-1||typeof sindex=='undefined')) {
   ...
   var a=(new JAX.ajax).load((document.location.toString().match('/acp/')?'../':'')+'misc/listloader.php?'+a,
   function(xml){
    var x,tmp
    ...
    for(x=0;x<xml[0].length;x++){
     ...
     tmp.key=xml[0][x]
     tmp.onclick=function(){this.parentNode.style.display='none';if(dummy) {dummy.value=this.key;if(dummy.onchange)dummy.onchange();};el.value=this.innerHTML;}
     d.appendChild(tmp)
    }
 ...
 this.flashTitle=function(a){
  ...
  JAX.flashInterval=setInterval(function(){
   document.title=(document.title==document.titleorig?document.title2:document.titleorig);
   },1000);
 }
 ...
 this.gracefulDegrade=function(a){
  ...
  for(x=0;x<l;x++) {
   if(links[x].href){
    if(links[x].getAttribute('href').charAt(0)=="?") {
     ...
     links[x].onclick=function(){if(!this.oldclick||this.oldclick()!=false) RUN.stream.location(this.getAttribute('href'));return false;}
  ...
  if(tmp=$$('.date',a)) {
   ...
   for(var x=0;x<tmp.length;x++) {
    ...
    tmp[x].onclick=function(){JAX.datepicker.init(this);}
 ...
 this.onImagesLoaded=function(imgs,f,timeout){
  var x,dbj={
   imgs:[],imgsloaded:1,called:false,
   force:function(){if(!dbj.called)f();}
  }
  dbj.callback=function(event){if(dbj.called) return;var x=dbj.imgs.inArray(this.src);if(x===false) return;dbj.imgs.splice(x,1);if(dbj.imgs.length==0){f();dbj.called=true;}}
  ...
  if(!imgs.length) {f();dbj.called=true;}
  else if(timeout) setTimeout(dbj.force,timeout);
 }

 this.handleTabs=function(e,a,f){
  var e=(e||window.event),el=e.target||e.srcElement,act
 ...
 this.collapse=function(a){
  var s=a.style,
   ...
   s.overflow='hidden'
  if(s.height=='0px') {
   JAX.sfx(a,5,10,0).add('height','0px',fh).then(function(){JAX.el.removeClass(b,'collapsed')}).play()
  } else {
   if(!fh) {
    fh=(a.clientHeight||a.offsetHeight)+"px";
    a.setAttribute('fullHeight',fh)
   }
   JAX.sfx(a,5,10,0).add('height',fh,'0px').then(function(){JAX.el.addClass(b,'collapsed')}).play()
  }
 }
...
/********************************************************************/
 ...
 this.el={
  ...
  getHighestZIndex:function(){
   ...
   for(x=0;x<l;x++) if(a[x].style.zIndex&&Number(a[x].style.zIndex)>max) max=Number(a[x].style.zIndex)
   return max+1
  }
 }
/********************************************************************/
 ...
 this.SWF=function(url,name,settings){
  ...
  embed+='></embed>';
  object+='</object>'
  var tmp=document.createElement('span');tmp.innerHTML=JAX.browser.ie?object:embed;
...
/********************************************************************/
 ...
 this.color=function(a){
  ...
  if (typeof a=="object") this.rgb=a
   ...
   tmp[1]=parseFloat(tmp[1]);tmp[2]=parseFloat(tmp[2]);tmp[3]=parseFloat(tmp[3]);
   tmp.shift();this.rgb=tmp;
  //HEX
   ...
   else {this.rgb=[];for(x=0;x<3;x++) this.rgb[x]=parseInt(a.substr(x*2,2),16);}
  } else this.rgb=[0,0,0]

  this.invert=function(){this.rgb=[255-this.rgb[0],255-this.rgb[1],255-this.rgb[2]];return this;}
  this.toRGB=function(){return this.rgb;}
  this.toHex=function(){
   if (!this.rgb) return false
   var tmp2,tmp="",x,hex="0123456789ABCDEF"
   for(x=0;x<3;x++) {
    tmp2=this.rgb[x]
    tmp+=hex.charAt(Math.floor(tmp2/16))+hex.charAt(Math.floor(tmp2%16))
   }
   return tmp
  }
  return this
...
/********************************************************************/
 ...
 this.sfx=function(el,steps,delay,loop){
  var tmp,x,y,me=this
  if(JAX==me) me={}
  me.el=el;me.steps=steps||30;me.delay=delay||20;me.stepCount=me.curLineup=0;me.loop=loop||0;
  me.lineup=[[]]

  me.play=function(){
   me.interval=setInterval(function(){me.step();},me.delay)
   return this
  }
  me.morph=function(from,percent,to){
   var x,r
   if (JAX.isArray(from)&&from.length==to.length) {r=[];for(x=0;x<from.length;x++) r[x]=Math.round(this.morph(from[x],percent,to[x]));}
   else {r=(to-from)*percent+from;}
   return r;
  }
  me.step=function(){
   var curL=me.lineup[me.curLineup],tmp,sc=me.stepCount++,tmp2,x
   if(typeof curL[0]=="function"){
    ...
    for(x=0;x<curL.length;x++) {
     ...
     if(tmp[0].match(/color/i)) {tmp2="#"+JAX.color(tmp2).toHex();}
  ...
  me.add=function(what,from,to){
   var t=['','','']
   if(what.match(/color/i)) {from=JAX.color(from).toRGB();t[1]=JAX.color(to).toRGB();}
   else {(t=to.match(/(\D*)(\-?\d+)(\D*)/)).shift();from=parseFloat(from.match(/\-?\d+/));}
   me.lineup[me.lineup.length-1].push([what,from,t[1],t[0],t[2]]);
   return this
  }
  me.dehighlight=function(){
   me.el.style.backgroundColor=''
   var bg=JAX.el.getComputedStyle(me.el).backgroundColor.toString(),bg2
   ...
   return me.add('backgroundColor',bg2,bg).then(function(){me.el.style.backgroundColor=bg;})
  }
...
/********************************************************************/
 ...
 this.editor=function(textarea,iframe){
  if(!iframe.timedout) {iframe.timedout=true;setTimeout(function(){JAX.editor(textarea,iframe)},100);return;}
  if(iframe.editor) return;
  var me={}
  me.iframe=iframe
  iframe.editor=me
  iframe.className="editorframe"
  me.mode=(JAX.browser.mobile||JAX.browser.n3ds?0:globalsettings.wysiwyg) //1 for html editing mode, 0 for textarea mode
  me.mode=me.mode||0
  ...
  me.buildEditBar=function(){
   ...
   var cmds=['bold','italic','underline','strikethrough','forecolor','backcolor','insertimage','createlink',
            'c_email','justifyleft','justifycenter','justifyright','c_youtube',
            'c_code','c_quote','c_spoiler','insertorderedlist','insertunorderedlist',
            'c_smileys','c_switcheditmode'],
       cmddesc=['Bold','Italic','Underline','Strike-Through','Foreground Color','Background Color','Insert Image','Insert Link',
               'Insert email','Align left','Center','Align right','Insert video from any of your favorite video services!',
               'Insert code','Insert Quote','Insert Spoiler','Create Ordered List','Create Unordered List',
               'Insert Emoticon','Switch editor mode'],
       l=cmds.length,a,x
   for(x=0;x<l;x++) {
	...
	a.unselectable='on'
    a.onclick=function(event){me.editbarCommand(event,this.className);}
  ...
  me.editbarCommand=function(e,cmd){
   ...
   if(cmd=="forecolor"||cmd=="backcolor") me.showColors(e.pageX,e.pageY,cmd)
  ...
  me.showEmotes=function(x,y){
   ...
   if(!emotewin){
    me.createEmoteWindow.x=x;me.createEmoteWindow.y=y;
  ...
  me.createEmoteWindow=function(xml){
   var rs=eval(xml.responseText),x,html,emotewin=document.createElement('div'),r,t
   emotewin.className='emotewin'
   for(x=0;x<rs[0].length;x++) {
    r=document.createElement('a')
    r.href='javascript:void(0)'
    r.emotetext=rs[0][x]
    r.onclick=function(){me.cmd('inserthtml',this.emotetext);me.hideEmotes();}
  ...
  me.showColors=function(posx,posy,cmd){
   if(me.colorWindow&&me.colorWindow.style.display!='none') return me.hideColors();
   var colorwin=me.colorWindow,
       colors=['FFFFFF','AAAAAA','000000','FF0000','00FF00','0000FF','FFFF00','00FFFF','FF00FF'],
       l=colors.length,sq=Math.ceil(Math.sqrt(l)),r,c,a
   if(!colorwin){
    ...
    for(y=0;y<sq;y++) {
     ...
     for(x=0;x<sq;x++){
      c=r.insertCell(x);if(!colors[x+y*sq]) continue;c.style.border='1px solid #000';c.style.padding=0;a=document.createElement('a');a.href='javascript:void(0)';a.onclick=me.colorHandler;c.appendChild(a);c=a.style;c.display='block';c.backgroundColor="#"+colors[x+y*sq];c.height=c.width='20px';c.margin=0;
     }
    }
   ...
   colorwin.style.top=posy+"px";colorwin.style.left=posx+"px"
  ...
  me.cmd=function(a,b,c){
   ...
   switch(a){
    case "bold":bbcode="[b]"+selection+"[/b]";break;
    ...
    case "insertorderedlist":
     if(!me.mode) bbcode="[ol]"+selection.replace(/(.+([\r\n]+|$))/ig,'*$1')+"[/ol]"
    break;
    case "insertunorderedlist":
     if(!me.mode) bbcode="[ul]"+selection.replace(/(.+([\r\n]+|$))/ig,'*$1')+"[/ul]"
    break;
    case "createlink":
    ...
    break;
    case "c_code":
     a="inserthtml";bbcode=b='[code]'+selection+'[/code]'
    break;
    case "c_quote":
     a="inserthtml";
  ...
  me.getSelection=function(){
   if(me.mode) return JAX.browser.ie?me.doc.selection.createRange().text:me.window.getSelection();
   else {
    if(JAX.browser.ie) {
     me.textarea.focus()
     return document.selection.createRange().text
    } else {
     return me.textarea.value.substring(me.textarea.selectionStart,me.textarea.selectionEnd)
    }
   }
  }
  ...
  me.BBtoHTML=function(a){
   a=a.replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/(\s) /g,"$1&nbsp;")
   a=me.replaceAll(a,/\[(b|i|u|s)\]([\w\W]*?)\[\/\1\]/ig,"<$1>$2</$1>")
   a=me.replaceAll(a,/\[img\]([^'"\[]+)\[\/img\]/ig,'<img src="$1">')
   a=me.replaceAll(a,/\[color=([^\]]+)\](.*?)\[\/color\]/ig,'<span style="color:$1">$2</span>')
   a=me.replaceAll(a,/\[size=([^\]]+)\](.*?)\[\/size\]/ig,'<span style="font-size:$1">$2</span>')
   a=me.replaceAll(a,/\[url=([^\]]+)\](.*?)\[\/url\]/ig,'<a href="$1">$2</a>')
   a=me.replaceAll(a,/\[bgcolor=([^\]]+)\](.*?)\[\/bgcolor\]/ig,'<span style="backgroun-color:$1">$2</span>')
   a=me.replaceAll(a,/\[h(\d)\](.*?)\[\/h\1\]/,'<h$1>$2</h$1>')
   a=me.replaceAll(a,/\[align=(left|right|center)\](.*?)\[\/align\]/,'<span style="text-align:$1">$2</span>')
   a=me.replaceAll(a,/\[(ul|ol)\]([\w\W]*?)\[\/\1\]/ig,function(s){
     var tag=RegExp.$1,lis="",list=RegExp.$2.split(/([\r\n]+|^)\*/),x;
     for(x=0;x<list.length;x++) if(list[x].match(/\S/)) lis+='<li>'+list[x]+'</li>'
     return '<'+tag+'>'+lis+'</'+tag+'>';
    })
   a=me.replaceAll(a,/\n/g,"<br />");
   return a
  ...
  me.HTMLtoBB=function(a){
   a=a.replace(/[\r\n]+/g,"")
   a=a.replace(/<(hr|br|meta)[^>]*>/ig,"\n")
   a=a.replace(/<img.*?src=["']?([^'"]+)["'][^>]*\/?>/g,"[img]$1[/img]");
   a=me.replaceAll(a,/<(\w+)([^>]*)>([\w\W]*?)<\/\1>/ig,function(whole,tag,attributes,innerhtml){
     var att={},style=''
     attributes.replace(/(color|size|style|href|src)=(['"]?)(.*?)\2/gi,function(whole,attr,q,value){att[attr]=value});

      if(att.style) style=att.style

     tag=tag.toLowerCase()
     if(tag=="script"||tag=="style"||tag=="hr") return
     if(style.match(/background(\-color)?:[^;]+(rgb\([^\)]+\)|#\s+)/i)) innerhtml="[bgcolor=#"+(new JAX.color(RegExp.$2)).toHex()+"]"+innerhtml+"[/bgcolor]"
     if(style.match(/text\-align: ?(right|center|left);/i)) innerhtml="[align="+RegExp.$1+"]"+innerhtml+"[/align]"
     if(style.match(/font\-style: ?italic;/i)||tag=="i"    ||tag=="em") innerhtml="[I]"+innerhtml+"[/I]"
     if(style.match(/text\-decoration:[^;]*underline;/i)      ||tag=="u") innerhtml="[U]"+innerhtml+"[/U]"
     if(style.match(/text\-decoration:[^;]*line\-through;/i)||tag=="s") innerhtml="[S]"+innerhtml+"[/S]"
     if(style.match(/font\-weight: ?bold;/i)||tag=="strong"||tag=="b") innerhtml="[B]"+innerhtml+"[/B]"
     if(att.size||style.match(/font\-size: ?([^;]+)/i)) innerhtml='[size='+(att.size||RegExp.$1)+']'+innerhtml+'[/size]'
     if(att.color||style.match(/color: ?([^;]+)/i)) innerhtml="[color="+(att.color||RegExp.$1)+']'+innerhtml+'[/color]'
     if(tag=="a"&&att.href) innerhtml='[url='+att.href+']'+innerhtml+'[/url]'
     if(tag=="ol") innerhtml="[ol]"+innerhtml+"[/ol]"
     if(tag=="ul") innerhtml="[ul]"+innerhtml+"[/ul]"
     if(tag.match(/h\d/i)) innerhtml="["+tag.toLowerCase()+"]"+innerhtml+"[/"+tag.toLowerCase()+"]"
     if(tag=="li") innerhtml='*'+innerhtml.replace(/[\n\r]+/,'')+"\n"
  ...
  me.submit=function(){
   if(me.mode) {me.switchMode(0);me.switchMode(1);}
  }
  setTimeout(function(){me.setSource(me.BBtoHTML(textarea.value));me.switchMode(me.mode)},100)
  return me
 }
...
/********************************************************************/
 ...
 this.scrollTo=function(pos,el){ //make this animate/not animate later based on preferences
 ...
 this.drag=function(){
  ...
  me.priv={
   start:function(e,t,handle){
    e=(new JAX.event(e)).cancel().stopBubbling();var el=t||this,s=JAX.el.getComputedStyle(el),highz=JAX.el.getHighestZIndex()
    ...
    if(!me.sess.zIndex||Number(me.sess.zIndex)<(highz-1)) el.style.zIndex=highz
   ...
   drag:function(e){
    ...
    var s=me.sess.el.style,sess,tmp=false,tx,ty,tmp2
    ...
    me.sess.info=sess={left:left,top:top,e:e,el:me.sess.el,mx:mx,my:my,droptarget:me.priv.testdrops(tx,ty),dx:mx-(sess.mx||mx),dy:my-(sess.my||my),self:me,sx:me.sess.ex,sy:me.sess.ey};
    ...
    if(typeof me.ondragout=='function'&&tmp&&sess['droptarget']!=tmp){tmp2=sess['droptarget'];sess['droptarget']=tmp;me.ondragout(sess);sess['droptarget']=tmp2;}
   },
   drop:function(){
   ...
   testdrops:function(a,b){
    var x,d=me.droppables,z,r=false,max=[9999,9999]
    if (!d) return r
    for(x=0;x<d.length;x++) {
	 if(d[x]==me.sess.el||JAX.el.isChildOf(d[x],me.sess.el)) continue;
	 z=JAX.el.getCoordinates(d[x]);
	 if(max[0]>z.w&&max[1]>z.h&&a>=z.x&&b>=z.y&&a<=z.xw&&b<=z.yh){max=[z.w,z.h];r=d[x];}
	}
    return r
   }
  }
  me.drops=function(a){this.droppables=a;return this;}
  me.addDrops=function(a){if(!this.droppables) {return this.drops(a);} this.droppables=this.droppables.concat(a);return this;}
  me.addListener=function(a){extend(this,a);return this;}
  me.apply=function(el,t){var x;if(el[0]){for(x=0;x<el.length;x++) me.apply(el[x]);return me;}var pos=JAX.el.getComputedStyle(el,'');pos=pos.position;if(!pos||pos=='static')el.style.position='relative';(t||el).onmousedown=t?function(e){me.priv.start(e,el,this);}:me.priv.start;return this;}
  me.boundingBox=function(x,y,w,h){this.bounds=[x,y,w,h];return this;}
  me.autoZ=function(){this._autoz=true;return this;}
  me.noChildActivation=function(){this._nochild=true;return this;}
  me.reset=function(el,zero){if(!el) el=me.sess.el;if(zero){el.style.top=el.style.left=0;} else { el.style.top=me.sess.ey+'px';el.style.left=me.sess.ex+'px';el.style.zIndex=me.sess.zIndex;} return me;}
 }
...
/********************************************************************/
 ...
 this.sortable=function(a,b){
  ...
  me.ondrop=function(a){if(me.change)me.coords=[];me.change=0;var s=a.el.style;s.top=s.left=0;if(typeof me.onend=='function') me.onend(a);}
  me.ondrag=function(a){
   var x, y, d=me.elems, dl=d.length, pos=0, c, cel=JAX.el.getCoordinates(a.el), c2, ch=false, ov=me.options.vertical||0,oh=me.options.horizontal||0, index
   if(!me.coords.length) {for(x=0;x<dl;x++) me.coords.push(JAX.el.getCoordinates(d[x]));}
   for(x=0;x<dl;x++){
   if(a.el==d[x]){index=x;break;}
    c=me.coords[x]
    if(ch===false&&(ov?a.my<c.yh&&a.dy<0:a.mx<c.xw&&a.my<c.yh)) {JAX.el.insertBefore(a.el,d[x]);ch=x;}
   }
   if (ch===false) {for(x=dl-1;x>=index;x--) {
    if(a.el==d[x])continue;
    c=me.coords[x]
    if((ov?a.my>c.y&&a.dy>0:a.mx>c.x&&a.my>c.y)) {JAX.el.insertAfter(a.el,d[x]);if(d.swap)me.elems=d.swap(index,x);ch=1;break;}
   }} else if (d.swap)me.elems=d.swap(index,ch);
   if(ch!==false){me.coords=[];me.change=1;c2=JAX.el.getCoordinates(a.el);me.sess.ex-=(c2.x-cel.x);me.sess.ey-=(c2.y-cel.y);me.priv.drag(a.e);}
   return false
  }
  for(var x=0;x<a.length;x++) {me.apply(a[x],typeof b.handle=='function'?b.handle(a[x]):null)}
 }
/*******************************************************************/
 this.sortableTree=function(tree,prefix,formfield){
 var tmp=$$('li',tree),x,items=[],seperators=[],all=[],drag
 if(formfield) formfield=$(formfield)
 for(x=0;x<tmp.length;x++) if(tmp[x].className!="title") items.push(tmp[x])

 function parsetree(tree){
  var nodes=tree.getElementsByTagName('li')
  var order={},node,sub,gotsomethin=0
 ...
 drag.drops(seperators.concat(items)).addListener({
  ondragover:function(a){a.droptarget.style.border='1px solid #000';},
  ondragout:function(a){a.droptarget.style.border='none';},
  ondrop:function(a){
   var next=a.droptarget.nextSibling,tmp,tmp2
   ...
   if(a.droptarget.className=='seperator') {
    if(parentlock&&a.droptarget.parentNode!=a.el.parentNode) return drag.reset(a.el,1);
    if(nofirstlevel&&a.droptarget.parentNode.className=='tree') return drag.reset(a.el,1);
    if(JAX.el.isChildOf(a.droptarget,a.el)||a.el==next) return drag.reset(a.el,1);
    if(next.className=="spacer") {next.parentNode.removeChild(next);}
    if(next.className!="spacer") JAX.el.insertAfter(a.el.previousSibling,a.droptarget);
    else a.el.previousSibling.parentNode.removeChild(a.el.previousSibling)
...
/*********************************************************************/
 ...
 this.event=function(e){
  ...
  var dB=document.body
  switch(e.keyCode){
   case 13:e.ENTER=true;break;
   case 37:e.LEFT=true;break;
   case 38:e.UP=true;break;
   case.39:e.RIGHT=true;break;
   case 40:e.DOWN=true;break;
  }
...
/********************************************************************/
 ...
 this.ajax=function(s){
  ...
  me.setup={readyState:4,callback:function(){},method:'POST'}
  ...
  me.load=function(a,b,c,d,e){
   ...
   if(d) d="POST"
   if(c&&JAX.isArray(c)&&JAX.isArray(c[0])&&c[0].length==c[1].length) c=me.build_query(c[0],c[1])
...
/********************************************************************/
 ...
 this.tooltip=function(el){
  var tooltip=document.getElementById('tooltip_thingy'),pos=JAX.el.getCoordinates(el),
  text=el.oldtitle=el.title.striphtml();el.title='';
  if(!text) return;
  if(!tooltip) {
   ...
   t.className="top";c.className="content";b.className="bottom"
   a=t.insertCell(0);a.className="left";a.colSpan=2;
   a=t.insertCell(1);a.className="right";
   a=c.insertCell(0);a.className="left";
   a=c.insertCell(1);a.innerHTML="default text";
   a=c.insertCell(2);a.className="right";
   a=b.insertCell(0);a.className="left";a.colSpan=2;
   a=b.insertCell(1);a.className="right";
...
this.imageResizer=function(imgs){
 ...
 for(var x=0;x<imgs.length;x++){
  p=p2=1
  img=imgs[x],nw=iw=parseInt(img.naturalWidth),nh=ih=parseInt(img.naturalHeight)
...
this.makeResizer=function(iw,nw,ih,nh,img){
   ...
   c.nw=nw;c.nh=nh;
   c.onmousemove=function(e){var o=JAX.el.getCoordinates(this);e=JAX.event(e);this.scrollLeft=(e.pageX-o.x)/o.w*(this.nw-o.w);this.scrollTop=(e.pageY-o.y)/o.h*(this.nh-o.h)}
   c.onmouseover=function(){img.style.width=this.nw+"px";img.style.height=this.nh+"px";}
   c.onmouseout=function(){if(this.scrollLeft) this.scrollLeft=this.scrollTop=0;img.style.width=iw+"px";img.style.height=ih+"px";}
   c.onmouseout()
...
/********************************************************************/
 ...
 this.window=function(){
  ...
  me.create=function(){
   ...
   if(me.wait) JAX.onImagesLoaded(d1.getElementsByTagName('img'),function(){me.setPosition(pos)},2000);
   else me.setPosition(pos)

   me.drag=(new JAX.drag).autoZ().noChildActivation().boundingBox(0,0,dE.clientWidth-50,dE.clientHeight-50).apply(d1,d2);
   d1.close=me.close
  ...
  me.close=function(){
   ...
   if(me.animate&&false) {
    ...
    .then(function(){document.body.removeChild(me.open);me.open=null;}).play()
   } else {
  ...
  me.minimize=function(){
   ...
   if (JAX.el.hasClass(c,"minimized")) {
    ...
    var wins=$$('.window');if(!wins.length) wins=[wins];
    for(x=0;x<wins.length;x++) if(JAX.el.hasClass(wins[x],'minimized')) w+=parseInt(wins[x].clientWidth)
  ...
  me.setPosition=function(pos,animate){
   var d1=me.open,x=0,y=0,cH=dE.clientHeight,cW=dE.clientWidth
 ...
 this.datepicker=new function(){
  ...
  this.generate=function(year,month,day){
   ...
   for(x=0;x<numdaysinmonth;x++){
    ...
    cell.onclick=function(){JAX.datepicker.insert(this);}
    cell.className='day'+(year==this.selectedDate[0]&&month==this.selectedDate[1]&&(x+1)==this.selectedDate[2]?' selected':'')
    cell.innerHTML=(x+1)
   }
  }
...
var Uploader=new function(){
 ...
 this.listenerHandler=function(id,action,args){
  ...
  switch (action){
   case "addfile":args[0].id=args[1];args=args[0];args.upload=function(url){Uploader.upload(id,this,url);};args=[args];break;
   case "startupload":args[0].id=args[1];args=[args[0]];break;
   case "progress":args[0].id=args[1];args.splice(1,1);break;
   case "error":args[2].id=args.pop();break;
   default:if(!args.length) args=[args];break;
  }
  if(this.uploaders[id]&&this.uploaders[id][action]) this.uploaders[id][action].apply(this.uploaders[id],args)
 }
 this.createButton=function(){
  var d=document.createElement('div');d.className='uploadbutton';d.innerHTML='Add File(s)';
  return [d,this.create(d)]
 }
 this.create=function(el,w,h,url){
  var nid=this.uploaders.length;
  var swf=JAX.SWF("Script/uploader.swf","uploader"+nid,{width:w||'100%',height:h||'100%',allowScriptAccess:'sameDomain','wmode':'transparent','flashvars':'id='+nid}),
      s=swf.style;
 ...
 this.upload=function(nid,fileobj,url){this.uploaders[nid].flashObj.upload(fileobj.id,url);}
}
...
var Sound=new function(){
 ...
 this.ready=function(){
  ...
  var q=me.queue
  me.flashObject=window.soundLoader&&window.soundLoader.playSound?window.soundLoader:document.soundLoader;
  if(!me.flashObject||!me.flashObject.loadSound) return; //no flash installed
  if(q.length) for(var x=0;x<q.length;x++) me.flashObject.loadSound(q[x][0],q[x][1],q[x][2]||false);
  me.queue=[]
  me.isready=true
  },100)
 }
 this.load=function(title,file,autoplay){
   ...
   if(this.isready) this.flashObject.loadSound(title,file,autoplay||false)
 ...
 this.addFlash=function(){
  document.body.appendChild(JAX.SWF("Script/soundLoader.swf","soundLoader",{hidden:'true',allowScriptAccess:'sameDomain',width:0,height:0}));
 }
}
