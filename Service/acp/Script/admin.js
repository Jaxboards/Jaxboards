function dd_menu(e) {
  ...
  if (el.tagName.toLowerCase() == "a") {
    ...
    el.onmouseout = function (e) {
      ...
      if (e.relatedTarget != menu && e.relatedTarget.offsetParent !=
        menu) {
    ...
    menu.onmouseout = function (e) {
      e = e || window.event
      if (!e.relatedTarget && e.toElement) e.relatedTarget = e.toElement;
      if (e.relatedTarget != el && e.relatedTarget.offsetParent !=
        menu && e.relatedTarget != menu) {
        JAX.el.removeClass(el, "active")
        menu.style.display = 'none'
      }
    }
...
function makestuffcool() {
  ...
  for (x = 0; x < l; x++) {
    ...
    s.set = function (onoff) {
      ...
      this.checked = onoff;
      this.t.style.backgroundPosition = this.checked ? 'top' :
        'bottom';
