   ...
   default:$this->showperms(); break;
    ...
    public function updateperms($perms)
    {
        ...
        foreach ($perms as $k => $v) {
            ...
            if ($k) {
                $DB->safeupdate('member_groups', $v, 'WHERE id=?', $k);
            }
        }
    ...
    public function showperms()
    {
        ...
        if (preg_match('@[^\\d,]@', @$JAX->b['grouplist']) || false !== strpos(@$JAX->b['grouplist'], ',,')) {
            $JAX->b['grouplist'] = '';
        }
...
<table style='padding-left:150px;width:100%;position:relative;background:#FFF' id='heading'>
    <tr>";
        foreach ($perms as $k => $v) {
            $page .= "<th style='width:".((1 / $numgroups) * 100)."%'><a class='icons edit' href='?act=groups&edit=${k}'>".$v['title']."</a> (${k})</th>";
        }
        $page .= "</tr>
  </table>
  <table class='perms'>
   ...
   array(
       ...
       'can_im' => 'Can IM',
            ...
            if ('breaker' == substr($k, 0, 7)) {
                $page .= "<tr><td class='breaker' colspan='".(1 + $numgroups)."'>${v}</td></tr>";
            } else {
                $page .= "<tr><td style='width:150px'>".$v.'</td>';
                foreach ($perms as $k2 => $v2) {
                    $page .= '<td class="center"><input name="perm['.$k2.']['.$k.']" type="checkbox" '.($v2[$k] ? 'checked="checked" ' : '').'class="switch yn" /></td>';
                }
                $page .= '</tr>';
            }
        }
  ...
  <input type="submit" name="submit" value="'.($gid ? 'Edit' : 'Create').'" />
    ...
    public function delete()
    {
        ...
        if (!$found) {
            $page .= "You haven't created any groups to delete. (Hint: default groups can't be deleted)";
        }
