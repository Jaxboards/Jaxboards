...
class groups
{
    ...
    public function updateperms($perms)
    {
        ...
        $error = $DB->error();
        if ($error) {
            $PAGE->addContentBox('Error', $PAGE->error($error));
        } else {
            $PAGE->addContentBox('Success!', $PAGE->success('Changes Saved successfully.'));
        }
    ...
    public function showperms()
    {
        ...
        if (! $numgroups) {
            $PAGE->addContentBox('Error', $PAGE->error("Don't play with my variables!"));
        }
