...
class themes
{
    public function __construct()
    {
        global $PAGE,$JAX;
        $sidebar = '';
        foreach (array('?act=themes&page=create' => 'Create New Skin', '?act=themes' => 'Manage Skins') as $k => $v) {
    ...
    public function showskinindex()
    {
        ...
        if (@$JAX->g['deletewrapper']) {
            if (!preg_match('@[^\\w ]@', $JAX->g['deletewrapper']) && file_exists(BOARDPATH.'Wrappers/'.$JAX->g['deletewrapper'].'.txt')) {
                ...
                $errorwrapper = 'The wrapper you are trying to delete does not exist.';
            }
        }

        if (@$JAX->p['newwrapper']) {
            if (preg_match('@[^\\w ]@', $JAX->p['newwrapper'])) {
                $errorwrapper = 'Wrapper name must consist of letters, numbers, spaces, and underscore.';
            } elseif (strlen($JAX->p['newwrapper']) > 50) {
                $errorwrapper = 'Wrapper name must be less than 50 characters.';
            } elseif (file_exists(BOARDPATH.'Wrappers/'.$JAX->p['newwrapper'].'.txt')) {
        ...
        if (@$JAX->p['submit']) {
            ...
            if (isset($JAX->p['renamewrapper']) && is_array($JAX->p['renamewrapper'])) {
                foreach ($JAX->p['renamewrapper'] as $k => $v) {
                    ...
                    if (preg_match('@[^\\w ]@', $v) || strlen($v) > 50) {
                        $errorwrapper = 'Wrapper name must consist of letters, numbers, spaces, and underscore, and be under 50 characters long.';
        ...
        while ($f = $DB->row($result)) {
            ...
            foreach ($wrappers as $v) {
                $skins .= "<option value='${v}' ".($v == $f['wrapper'] ? "selected='selected' " : '').">${v}</option>";
            }
...
heredoc;
  ...
  <div class='center'>".($skin['custom'] ? "<input type='submit' value='Save Changes' />" : '').'</div>
        ...
        if (preg_match('@[^ \\w]@', $wrapper) && !is_file($wrapperf)) {
            ...
            if (isset($JAX->p['newwrapper'])) {
                if (false === strpos($JAX->p['newwrapper'], '<!--FOOTER-->')) {
                    $saved = $PAGE->error('&lt;!--FOOTER--&gt; must not be removed from the wrapper.');
                } else {
                    $o = fopen($wrapperf, 'w');
    ...
    public function deleteskin($id)
    {
        ...
        if ($skin['default']) {
            $DB->safeupdate('skins', array('default' => 1), 'LIMIT 1');
        }
        $PAGE->location('?act=themes');
    }
}
