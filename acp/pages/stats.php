...
class stats{
 ...
 function recount_statistics(){
  global $DB,$PAGE;
  $DB->select("id,nocount","forums");
  while($f=$DB->row()) $pc[$f['id']]=$f['nocount'];
  $DB->special("SELECT p.id,p.auth_id,p.tid,t.fid FROM %t p JOIN %t t ON p.tid=t.id","posts","topics");
  ...
  $DB->select("id,path,cat_id","forums");
  while($f=$DB->row()){
  ...
  foreach($stat['topic_posts'] as $k=>$v) $DB->update("topics",Array("replies"=>$v),"WHERE id=$k");

  //Update member posts
  foreach($stat['member_posts'] as $k=>$v) $DB->update("members",Array("posts"=>$v),"WHERE id=$k");

  //Update forum posts
  foreach($stat['forum_posts'] as $k=>$v) $DB->update("forums",Array("posts"=>$v,"topics"=>$stat['forum_topics'][$k]),"WHERE id=$k");

  //get # of members
  $DB->select("count(id)","members");
  $stat['members']=array_pop($DB->row());

  //Update global board stats
  $DB->update("stats",Array("posts"=>$stat['posts'],"topics"=>$stat['topics'],"members"=>$stat['members']));
