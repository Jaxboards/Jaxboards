   ...
   default:$this->showstats(); break;
    ...
    public function recount_statistics()
    {
        ...
        foreach ($stat['topic_posts'] as $k => $v) {
            $DB->safeupdate('topics', array('replies' => $v), 'WHERE id=?', $k);
        }

        //Update member posts
        foreach ($stat['member_posts'] as $k => $v) {
            $DB->safeupdate('members', array('posts' => $v), 'WHERE id=?', $k);
        }

        //Update forum posts
        foreach ($stat['forum_posts'] as $k => $v) {
            $DB->safeupdate('forums', array('posts' => $v, 'topics' => $stat['forum_topics'][$k]), 'WHERE id=?', $k);
        }
