    ...
    public function filemanager()
    {
        ...
        if (is_array(@$JAX->p['dl'])) {
            foreach ($JAX->p['dl'] as $k => $v) {
                if (ctype_digit($v)) {
                    $DB->safeupdate('files', array('downloads' => $v), 'WHERE id=?', $DB->basicvalue($k));
                }
            }
        ...
        while ($f = $DB->row($result)) {
            ...
            foreach ($m[1] as $v) {
                $linkedin[$v][] = '<a href="../?act=vt'.$f['tid'].'&findpost='.$f['id'].'">'.$f['id'].'</a>';
            }
        }
        ...
        while ($file = $DB->row($result)) {
            ...
            if (in_array($ext, $CFG['images'])) {
                $file['name'] = '<a href="'.BOARDPATHURL.'Uploads/'.$file['hash'].'.'.$ext.'">'.$file['name'].'</a>';
            } else {
                $file['name'] = '<a href="../?act=download&id='.$file['id'].'">'.$file['name'].'</a>';
            }
