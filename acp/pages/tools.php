...
class tools{
 ...
 function filemanager(){
  ...
  if(is_numeric($JAX->b['delete'])){
   $DB->select("*","files","WHERE id=".$DB->evalue($JAX->b['delete']));
   $f=$DB->row();
   if($f){
   ...
   $DB->delete("files","WHERE id=".$DB->evalue($JAX->b['delete']));
   }
  }
  if(is_array($JAX->p['dl'])) {
   foreach($JAX->p['dl'] as $k=>$v) if(ctype_digit($v)) $DB->update("files",Array("downloads"=>$v),"WHERE id=".$DB->evalue($k));
   $page.=$PAGE->success('Changes saved.');
  }
  $DB->select("id,tid,post","posts","WHERE MATCH(post) AGAINST('attachment') AND post LIKE '%[attachment]%'");
  $linkedin=Array();
  while($f=$DB->row()){
   preg_match_all('@\[attachment\](\d+)\[/attachment\]@',$f['post'],$m);
   foreach($m[1] as $v) $linkedin[$v][]='<a href="../?act=vt'.$f['tid'].'&findpost='.$f['id'].'">'.$f['id'].'</a>';
  }
  $DB->special("SELECT f.*,m.display_name uname FROM %t f LEFT JOIN %t m ON f.uid=m.id ORDER BY f.size DESC","files","members");
  echo $DB->error();
  $table='';
  while($file=$DB->row()) {
 ...
 function backup(){
    ...
    if($_POST['dl']) {
        ...
        $tables=$DB->special("SHOW TABLES IN jaxboards_service LIKE 'blueprint_%%' ");
        $page="";
        //$o=fopen("backup.sql","w");
        while($f=$DB->row($tables)) {
            $f[0]=substr(strstr($f[0],"_"),1);
            $page.=$f[0];
            outline("--".$f[0]);
            $createtable=$DB->special("SHOW CREATE TABLE %t",$f[0]);
            outline(array_pop($DB->row($createtable)));
            $select=$DB->select("*",$f[0]);
