...
class members{
 ...
 function showmain(){
  global $PAGE,$DB,$JAX;
  $DB->special("SELECT m.id,m.avatar,m.display_name,m.group_id,g.title group_title FROM %t m LEFT JOIN %t g ON m.group_id=g.id ORDER BY m.display_name ASC","members","member_groups");
    ...
    if($JAX->p['savedata']){
      if($data['group_id']!=2||$JAX->userData['id']==1){
       ...
       foreach(Array(
        ...
        $DB->update("members",$write,"WHERE id=".$DB->evalue($JAX->b['mid']));
        $page=$PAGE->success("Profile data saved");
      } else $page=$PAGE->error("You do not have permission to edit this profile.".$PAGE->back());
    }
    $DB->select("*","members","WHERE id=".$DB->evalue($JAX->b['mid']));
   } else $DB->select("*","members","WHERE display_name LIKE ".$DB->evalue($JAX->p['name']."%"));
   $data=Array();
 ...
 function preregister(){
  ...
  $page="";
  if($JAX->p['submit']){
   if(!$JAX->p['username']||!$JAX->p['displayname']||!$JAX->p['pass']) $e="All fields required.";
   elseif(strlen($JAX->p['username'])>30||$JAX->p['displayname']>30) $e="Display name and username must be under 30 characters.";
   elseif($f=$DB->row($DB->select("name,display_name","members","WHERE name=".$DB->evalue($JAX->p['username'])." OR display_name=".$DB->evalue($JAX->p['displayname']))))
   
    $e="That ".($f['name']==$JAX->p['username']?"username":"display name")." is already taken";
   if($e) $page.=$PAGE->error($e);
   else {
    if($DB->insert("members",Array("name"=>$JAX->p['username'],"display_name"=>$JAX->p['displayname'],"pass"=>md5($JAX->p['pass']),"last_visit"=>time(),"group_id"=>1,"posts"=>0))) $page.=$PAGE->success("Member registered.");
 ...
 function getGroups($group_id=0){
  global $DB;
  $DB->select("id,title","member_groups","ORDER BY `title` DESC");
  while($f=$DB->row()) $page.="<option value='".$f['id']."'".($group_id==$f['id']?" selected='selected'":"").">".$f['title']."</option>";
 ...
 function merge(){
  ...
  if($JAX->p['submit']) {
   ...
   if($e) $page.=$PAGE->error($e);
    ...
    $mid1=$DB->evalue($JAX->p['mid1']);
    $mid1int=$JAX->p['mid1'];
    $mid2=$JAX->p['mid2'];
    
    //$DB->debug_mode();
    //files
    $DB->update('files',   Array('uid'=>    $mid2),'WHERE uid='.    $mid1);
    //PMs
    $DB->update('messages',Array('to'=>     $mid2),'WHERE to='.     $mid1);
    $DB->update('messages',Array('from'=>   $mid2),'WHERE from='.   $mid1);
     //posts
    $DB->update("posts",   Array('auth_id'=>$mid2),"WHERE auth_id=".$mid1);
    //profile comments
    $DB->update('profile_comments',Array('to'=>     $mid2),'WHERE to='.     $mid1);
    $DB->update('profile_comments',Array('from'=>   $mid2),'WHERE from='.   $mid1);
    //topics
    $DB->update("topics",  Array('auth_id'=>$mid2),"WHERE auth_id=".$mid1);
    $DB->update("topics",  Array('lp_uid'=>$mid2),"WHERE lp_uid=".$mid1);
    
    //forums
    $DB->update("forums",Array('lp_uid'=>$mid2),'WHERE lp_uid='.$mid1);
    
    //shouts
    $DB->update("shouts",  Array('uid'=>    $mid2),"WHERE uid=".    $mid1);
    
    //session
    $DB->update("session",Array('uid'=>$mid2),"WHERE uid=".$mid1);
    
    //arcade
    $DB->update("arcade_scores",Array('uid'=>$mid2),"WHERE uid=".$mid1);
    $DB->update("arcade_games",Array('leader'=>$mid2),"WHERE leader=".$mid1);
    
    //sum post count on account being merged into
    $DB->select("posts,id","members","WHERE id=".$mid1);
    $posts=$DB->row($q);
    if(!$posts) $posts=0; else $posts=$posts[0];
    $DB->update("members",Array('posts'=>Array('posts+'.$posts)),'WHERE id='.$mid2);
    
    //delete the account
    $DB->delete("members","WHERE id=".$mid1);
    
    //update stats
    $DB->update("stats",Array('members'=>Array('members-1'),'last_register'=>Array('(SELECT max(id) FROM '.$DB->prefix.'members)')));
 ...
 function deletemem(){
  ...
  if($JAX->p['submit']) {
   ...
   if($e) $page.=$PAGE->error($e);
    ...
    $mid=$DB->evalue($JAX->p['mid']);
    
    //PMs
    $DB->delete('messages','WHERE to='.     $mid);
    $DB->delete('messages','WHERE from='.   $mid);
    //posts
    $DB->delete("posts","WHERE auth_id=".$mid);
    //profile comments
    $DB->delete('profile_comments','WHERE to='.     $mid);
    $DB->delete('profile_comments','WHERE from='.   $mid);
    //topics
    $DB->delete("topics","WHERE auth_id=".$mid);
    
    //forums
    $DB->update("forums",Array("lp_uid"=>0,"lp_date"=>0,"lp_tid"=>0,"lp_topic"=>0),'WHERE lp_uid='.$mid);
    
    //shouts
    $DB->delete("shouts","WHERE uid=".    $mid);
    
    //session
    $DB->delete("session","WHERE uid=".$mid);
    
    //arcade
    $DB->delete("arcade_scores","WHERE uid=".$mid);
    //TODO: Fix arcade game leader of deleted member if they have high scores
    
    //delete the account
    $DB->delete("members","WHERE id=".$mid);
 ...
 function massmessage(){
  ...
  if($JAX->p['submit']){
   if(!trim($JAX->p['title'])||!trim($JAX->p['message'])) $page.=$PAGE->error("All fields required!");
    ...
    $q=$DB->select("id","members","WHERE (".time()."-last_visit)<".(60*60*24*31*6));
    $num=0;
    while($f=$DB->row($q)) {
     $DB->insert("messages",Array("to"=>$f['id'],"from"=>$JAX->userData['id'],"message"=>$JAX->p['message'],"title"=>$JAX->p['title'],"del_recipient"=>0,"del_sender"=>0,"read"=>0,"flag"=>0,"date"=>time()));
     $num++;
    }
 ...
 function validation(){
    ...
    if(isset($_POST['mid'])) {
        if($_POST['action']=="Allow") {
            $DB->update('members',Array('group_id'=>1),'WHERE id='.$DB->evalue($_POST['mid']));
        }
    }
    $page='';
    $DB->select('id,display_name,inet_ntoa(ip) ip,email,join_date','members','WHERE group_id=5');
    while($f=$DB->row()) {
