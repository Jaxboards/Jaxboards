...
class forums
{
    ...
    public function forums()
    {
        ...
        foreach ($links as $k => $v) {
            $sidebar .= '<li><a href="?act=forums&do='.$k.'">'.$v.'</a></li>';
        }
    ...
    public function orderforums($highlight = 0)
    {
        ...
        $forums = array();
        $result = $DB->safeselect('*', 'categories', 'ORDER BY `order`,id ASC');
        ...
        $result = $DB->safeselect('*', 'forums', 'ORDER BY `order`,title');
        $tree = array($result);
        while ($f = $DB->row($result)) {
            ...
            foreach ($treeparts as $v) {
                ...
                if (!is_array(@$intree[$v])) {
                    $intree[$v] = array();
                } /* BUGBUGBUG: Why does this sometimes generate warnings without the @? */
                $intree = &$intree[$v];
            }
            if (!@$intree[$f['id']]) {
                $intree[$f['id']] = true;
    ...
    public static function mysqltree($tree, $p = '', $x = 0)
    {
        ...
        foreach ($tree as $k => $v) {
            ...
            if ('c' == $k[0]) {
                $DB->safeupdate('categories', array('order' => $x), 'WHERE id=?', $cat);
            } else {
                $DB->safeupdate('forums', array('path' => preg_replace('@\\s+@', ' ', $f), 'order' => $x, 'cat_id' => $cat), 'WHERE id=?', $k);
            }
        }
    }
   ...
   ((@$data[$k]['trashcan']) ? '<span class="icons trashcan"></span>' : ''). /* BUGBUGBUG: Why does this sometimes generate warnings without the @? */
    $data[$k]['title'].
   ((@$data[$k]['mods']) ? ' - <i>'.($nummods = count(explode(',', $data[$k]['mods']))).' moderator'.(1 == $nummods ? '' : 's').'</i>' : ''). /* BUGBUGBUG: Why does this sometimes generate warnings without the @? */
    ...
    public function createforum($fid = false)
    {
        ...
        if (is_numeric(@$JAX->b['rmod'])) {
            ...
            if ($fdata['mods']) {
                ...
                $DB->safeupdate('forums', array('mods' => $fdata['mods']), 'WHERE id=?', $DB->basicvalue($fid));
        ...
        if (@$JAX->p['submit']) {
            ...
            if (is_numeric($JAX->p['modid'])) {
                $result = $DB->safeselect('*', 'members', 'WHERE id=?', $DB->basicvalue($JAX->p['modid']));
                if ($DB->row($result)) {
                    if (false === array_search($JAX->p['modid'], explode(',', $fdata['mods']))) {
            ...
            if (!$e) {
                ...
                if ($write['trashcan'] || (!$write['trashcan'] && @$fdata['trashcan'])) {
                    $DB->safeupdate('forums', array('trashcan' => 0));
                }
        ...
        {
            return '<input type="checkbox" class="switch yn" name="groups['.$id.']['.$name.']" '.($checked ? 'checked="checked" ' : '').('global' == $name ? ' onchange="globaltoggle(this.parentNode.parentNode,this.checked)" ' : '').'/>';
        }
...
<tr><td>Show Subforums:</td><td><select name='showsub'>";
        foreach (array('Not at all', 'One level below', 'All subforums') as $k => $v) {
            $page .= '<option value="'.$k.'"'.(($k == @$fdata['show_sub']) ? ' selected="selected"' : '').'>'.$v.'</option>';
        }
        $page .= "</select></td></tr>
<tr><td>Order Topics By:</td><td><select name='orderby'>";
        foreach (array('Last Post, Descending', 'Last Post, Ascending', 'Topic Creation Time, Descending', 'Topic Creation Time, Ascending', 'Topic Title, Descending', 'Topic Title, Ascending') as $k => $v) {
            $page .= "<option value='".$k."'".((@$fdata['orderby'] == $k) ? " selected='selected'" : '').'>'.$v.'</option>';
        }
        $page .= "</select></td></tr>
...
<tr><td>Moderators:</td><td>';
        if (@$fdata['mods']) {
            ...
            $mods = '';
            while ($f = $DB->row($result)) {
                $mods .= $f['display_name'].' <a href="?act=forums&edit='.$fid.'&rmod='.$f['id'].'">X</a>, ';
            }
...
for(var x=1;x<perms.rows.length;x++){
 globaltoggle(perms.rows[x],perms.rows[x].getElementsByTagName('input')[0].checked)
    ...
    {
        global $JAX,$DB,$PAGE;
        if ('Cancel' == $JAX->p['submit']) {
            $PAGE->location('?act=forums&do=order');
        } elseif ($JAX->p['submit']) {
            $DB->safedelete('forums', 'WHERE id=?', $DB->basicvalue($id));
            if ($JAX->p['moveto']) {
                $DB->safeupdate('topics', array('fid' => $JAX->p['moveto']), ' WHERE fid=?', $DB->basicvalue($id));
                $topics = $DB->affected_rows(1);
            } else {
                $result = $DB->safespecial('DELETE FROM %t WHERE tid IN (SELECT id FROM %t WHERE fid=? )',
    array('posts', 'topics'),
    $DB->basicvalue($id));
        ...
        $result = $DB->safeselect('*', 'forums', 'WHERE id=?', $DB->basicvalue($id));
        $fdata = $DB->row($result);
        $DB->disposeresult($result);
    ...
    public function createcategory($cid = false)
    {
        ...
        if (@$JAX->p['submit']) {
            if (!trim($JAX->p['cat_name'])) {
                ...
                if ($cdata) {
                    $DB->safeupdate('categories', $stuff, 'WHERE id=?', $DB->basicvalue($cid));
                } else {
  ...
  <input type="submit" name="submit" value="'.($cdata ? 'Edit' : 'Create').'" />
    ...
    public function deletecategory($id)
    {
        global $PAGE,$DB,$JAX;
        $page = '';
        $result = $DB->safeselect('*', 'categories');
        $categories = array();
        $cattitle = false;
        ...
        if (!$e && $JAX->p['submit']) {
            if (!isset($categories[$JAX->p['moveto']])) {
                ...
                $DB->safeupdate('forums', array('cat_id' => $JAX->p['moveto']), 'WHERE cat_id=?', $DB->basicvalue($id));
    ...
    {
        global $DB;
        $DB->safeupdate('members', array('mod' => 0));
        ...
        //update
        $DB->safeupdate('members', array('mod' => 1), 'WHERE id IN ?', array_keys($mods));
    }
}
