...
class settings{
 ...
 function wordfilter(){
  ...
  if(empty($wordfilter)) $table="<tr><td colspan='3'>Not currently filtering words.</td></tr>".$submit;
  else {
 ...
 function emoticons(){
  ...
  if(empty($emoticons)) $table="<tr><td colspan='3'>No custom emotes!</td></tr><tr><th>Emoticon</th><th>Image</th><th>In list?</th><th></th></tr>".$inputs;
  else {
 ...
 function postrating(){
  ...
  if(empty($niblets)) {
   ...
   krsort($niblets);
   foreach($niblets as $k=>$v) $table.="<tr><td><img src=\"".$JAX->blockhtml($v['img'])."\" /></td><td>".$JAX->blockhtml($v['title'])."</td><td class='x'><a href='?act=posting&do=postrating&d=".$k."'>X</a></td></tr>";
  }
