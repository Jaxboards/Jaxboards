...
class settings
{
    ...
    public function postrating()
    {
        ...
        $page = $page2 = '';
        $niblets = [];
        $result = $DB->safeselect('`id`,`img`,`title`', 'ratingniblets', 'ORDER BY `id` DESC');
        while ($f = $DB->arow($result)) {
            $niblets[$f['id']] = [
                'img' => $f['img'],
                'title' => $f['title'],
        ...
        $table = $PAGE->parseTemplate('posting/post-rating-heading.html');
        if (empty($niblets)) {
            $table .= $PAGE->parseTemplate('posting/post-rating-empty-row.html');
        } else {
            krsort($niblets);
