    ...
    $USER['id']
        ...
        if (!$hasmessages) {
            if ('sent' == $view) {
                ...
                $msg = 'No messages. You could always try <a href="?act=inbox&page=compose">sending some</a>, though!';
            }
            $page .= '<tr><td colspan="5" class="error">'.$msg.'</td></tr>';
        } else {
            $page .= '<tr><td></td><td colspan="4"><button>This button does nothing</button></td></tr>';
        }

        $page = $PAGE->meta('inbox-messages-listing', 'sent' == $view ? 'Recipient' : 'Sender', $page);
