...
class IDX
{
    ...
    public function getmods($modids)
    {
        ...
        if (!$this->moderatorinfo) {
            ...
            while ($f = $DB->row($result)) {
                $this->moderatorinfo[$f['id']] = $PAGE->meta('user-link', $f['id'], $f['group_id'], $f['display_name']);
            }
        }
    ...
    public function getusersonlinelist()
    {
        ...
        foreach ($DB->getUsersOnline() as $f) {
            if ($f['uid'] || $f['is_bot']) {
                ...
                if ($f['is_bot']) {
                    $r .= '<a class="user'.$f['uid'].'" title="'.$title.'" onmouseover="JAX.tooltip(this)">'.$f['name'].'</a>';
                } else {
                    ++$nummembers;
                    $r .= sprintf('<a href="?act=vu%1$s" class="user%1$s mgroup%2$s" title="%4$s" onmouseover="JAX.tooltip(this)">%3$s</a>',
                    $f['uid'],
                    $f['group_id'].('idle' == $f['status'] ? ' idle' : ($f['birthday'] && ($CFG['birthdays'] & 1) ? ' birthday' : '')),
            $f['name'],
            $title
          );
                }
    ...
    public function updateStats()
    {
        ...
        foreach ($DB->getUsersOnline() as $f) {
            if ($f['uid'] || $f['is_bot']) {
                if ($f['last_action'] >= $SESS->last_update || 'idle' == $f['status'] && $f['last_action'] > ($SESS->last_update - $CFG['timetoidle'] - 30)) {
                    $list[] = array($f['uid'], $f['group_id'], ('active' != $f['status'] ? $f['status'] : ($f['birthday'] && ($CFG['birthdays'] & 1) ? ' birthday' : '')), $f['name'], $f['location_verbose']);
                }
