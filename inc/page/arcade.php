...
class arcade
{
    ...
    public function play($id)
    {
        ...
        if ($JAX->b['frame']) {
            die('<html><head></head><body style="margin:0;padding:0">'.$PAGE->SWF($gamedata['swf'], array('width' => $gamedata['width'].'px', 'height' => $gamedata['height'].'px', 'flashvars' => array('jax_gameid' => $id))).'</body></html>');
        }
    ...
    public function showScores($gameid)
    {
        ...
        $page .= '<tr><th width="1"></th><th></th><th>Name</th><th>Score</th><th>Comment</th><th>Date</th>'.(2 == $USER['group_id'] ? '<th>X</th>' : '').'</tr>';
        foreach ($scores as $k => $f) {
            ...
            $page .= '</td><td>'.$JAX->date($f['date']).'</td>'.(2 == $USER['group_id'] ? '<td><a href="?act=arcade&amp;scores='.$gameid.'&del='.$f['id'].'">[X]</a></td>' : '').'</tr>';
        }
        $page .= '</table>';
