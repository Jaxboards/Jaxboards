...
class members
{
    ...
    public function showmemberlist()
    {
        ...
        $sortby = 'display_name';
        $sorthow = 'desc' == $JAX->b['how'] ? 'desc' : 'asc';
        if ($vars[$JAX->b['sortby']]) {
            $sortby = $JAX->b['sortby'];
        }
        if ('staff' == $JAX->g['filter']) {
    ...
    $this->perpage);
        ...
        foreach ($JAX->pages(ceil($nummembers / $this->perpage), $this->page + 1, $this->perpage) as $v) {
            $pages .= "<a href='?act=members&amp;sortby=${sortby}&amp;how=${sorthow}&amp;page=${v}'".($v - 1 == $this->page ? ' class="active"' : '').">${v}</a> ";
        }
        $url = '?act=members'.($this->page ? '&page='.($this->page + 1) : '').($JAX->g['filter'] ? '&filter='.$JAX->g['filter'] : '');
        $links = array();
        foreach ($vars as $k => $v) {
            $links[] = "<a href=\"${url}&amp;sortby=${k}".($sortby == $k ? ('asc' == $sorthow ? '&amp;how=desc' : '').'" class="sort'.('desc' == $sorthow ? ' desc' : '') : '')."\">${v}</a>";
        }
        foreach ($memberarray as $f) {
            ...
            foreach (array('skype' => 'skype:%s', 'msn' => 'msnim:chat?contact=%s', 'gtalk' => 'gtalk:chat?jid=%s', 'aim' => 'aim:goaim?screenname=%s', 'yim' => 'ymsgr:sendim?%s', 'steam' => 'https://steamcommunity.com/id/%s', 'twitter' => 'https://twitter.com/%s') as $k => $v) {
                if ($f['contact_'.$k]) {
                    $contactdetails .= '<a class="'.$k.' contact" href="'.sprintf($v, $JAX->blockhtml($f['contact_'.$k])).'">&nbsp;</a>';
                }
            }
