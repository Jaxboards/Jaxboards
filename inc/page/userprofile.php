...
class userprofile
{
    ...
    public $contacturls = array('skype' => 'skype:%s', 'msn' => 'msnim:chat?contact=%s', 'gtalk' => 'gtalk:chat?jid=%s', 'aim' => 'aim:goaim?screenname=%s', 'yim' => 'ymsgr:sendim?%s', 'steam' => 'https://steamcommunity.com/id/%s', 'twitter' => 'https://twitter.com/%s');
  ...
  FROM %t AS m LEFT JOIN %t AS g ON m.group_id=g.id WHERE m.id=?',
    ...
    $id);
        ...
        foreach ($this->contacturls as $k => $v) {
            if ($ud['contact_'.$k]) {
                $contactdetails .= '<a class="'.$k.' contact" href="'.sprintf($v, $JAX->blockhtml($ud['contact_'.$k])).'">&nbsp;</a>';
            }
        }
        $PAGE->JS('softurl');
        $PAGE->JS('window', array('useoverlay' => 1, 'minimizable' => false, 'animate' => false, 'title' => 'Contact Card', 'className' => 'contact-card', 'content' => $PAGE->meta(
   ...
   default:
    ...
    if ('RSS' == $JAX->b['fmt']) {
     ...
     if (!$pfbox) {
         ...
         $pfbox = "<a href='./?act=vu".$id."&amp;page=activity&amp;fmt=RSS' class='social rss' style='float:right'>RSS</a>".$pfbox;
     }
   ...
   case 'friends':
    ...
    exlode(',', $udata['friends']));
        ...
        while ($f = $DB->row($result)) {
            $pfbox .= $PAGE->meta('userprofile-friend', $f['id'], $JAX->pick($f['avatar'], $PAGE->meta('default-avatar')), $PAGE->meta('user-link', $f['id'], $f['group_id'], $f['name']));
        }
    }
    if (!$pfbox) {
   ...
   case 'comments':
    ...
    if ($USER && $PERMS['can_add_comments']) {
        $pfbox = $PAGE->meta('userprofile-comment-form',
     $USER['name'],
