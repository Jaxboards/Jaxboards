...
class userprofile{
 ...
 function showcontactcard($id){
  ...
  $DB->special("SELECT m.id AS uid,m.display_name AS uname,m.usertitle,g.title,m.avatar,
  m.contact_gtalk,m.contact_aim,m.contact_yim,m.contact_msn,m.contact_skype,m.contact_steam,m.contact_twitter
  FROM %t AS m LEFT JOIN %t AS g ON m.group_id=g.id WHERE m.id=$id","members","member_groups");
  $ud=$DB->arow();
 ...
 function showfullprofile($id){
  ...
  else $DB->special("SELECT m.*,g.title `group` FROM %t m LEFT JOIN %t g ON m.group_id=g.id WHERE m.id=".$id,"members","member_groups");
  echo $DB->error();
  $udata=$DB->row();
  if(!$udata||$nouser) {
  ...
  switch($pfpageloc){
   ...
   default:
    $pfpageloc="activity";
    $DB->special("SELECT a.*,a.affected_uid aff_id,m.display_name aff_name,m.group_id aff_group_id FROM %t a LEFT JOIN %t m ON a.affected_uid=m.id WHERE a.uid=$id ORDER BY a.id DESC LIMIT ".$this->num_activity,"activity","members");
    if($JAX->b['fmt']=="RSS") {
     ...
     while($f=$DB->row()) {$f['name']=$udata['display_name'];$f['group_id']=$udata['group_id'];$pfbox.=$JAX->parse_activity($f);}
   ...
   break;
   case "posts":
    $DB->special("SELECT p.post,p.id pid,p.tid,t.title,p.date,f.perms FROM %t p LEFT JOIN %t t ON p.tid=t.id LEFT JOIN %t f ON f.id=t.fid WHERE p.auth_id=$id ORDER BY p.id DESC LIMIT 10","posts","topics","forums");
    while($f=$DB->row()) {
   ...
   break;
   case "topics":
    $DB->special("SELECT p.post,p.id pid,p.tid,t.title,p.date,f.perms FROM %t p LEFT JOIN %t t ON p.tid=t.id LEFT JOIN %t f ON f.id=t.fid WHERE p.auth_id=$id AND p.newtopic=1 ORDER BY p.id DESC LIMIT 10","posts","topics","forums");
    while($f=$DB->row()) {
        $p=$JAX->parseperms($f['perms'],$USER?$USER['group_id']:3);
        if(!$p['read']) continue;
   ...
   case "friends":
    if($udata['friends']){
     $DB->special("SELECT m.avatar,m.id,m.display_name name,m.usertitle FROM %t m LEFT JOIN %t g ON m.group_id=g.id WHERE m.id IN(".$udata['friends'].") ORDER BY name","members","member_groups");

     while($f=$DB->row()) $pfbox.=$PAGE->meta('userprofile-friend',$f['id'],$JAX->pick($f['avatar'],$PAGE->meta('default-avatar')),$PAGE->meta('user-link',$f['id'],$f['group_id'],$f['name']));
    }
   ...
   case "comments":
    ...
    if(isset($JAX->p['comment'])&&$JAX->p['comment']!=="") {
     if(!$USER||!$PERMS['can_add_comments']) {
      ...
      $DB->insert("activity",Array("type"=>"profile_comment","uid"=>$USER['id'],"date"=>time(),"affected_uid"=>$id));
      $DB->insert("profile_comments",Array("to"=>$id,"from"=>$USER['id'],"comment"=>$JAX->p['comment'],"date"=>time()));
     }
     if($e) {
    ...
    $DB->special("SELECT c.*,m.display_name,m.group_id,m.avatar FROM %t c LEFT JOIN %t m ON c.from=m.id WHERE c.to=$id ORDER BY id DESC LIMIT 10","profile_comments","members");
    while($f=$DB->row()) {
