    ...
    public function showsoundsettings()
    {
        ...
        if ($JAX->p['submit']) {
            ...
            foreach ($variables as $v) {
                $update[$v] = $JAX->p[$v] ? 1 : 0;
            }
            $DB->safeupdate('members', $update, 'WHERE id=?', $USER['id']);

            foreach ($variables as $v) {
                $PAGE->JS('script', "window.globalsettings.${v}=".($JAX->p[$v] ? 1 : 0));
            }
        ...
        foreach ($variables as $v) {
            $checkboxes[] = '<input type="checkbox" name="'.$v.'" '.($USER[$v] ? 'checked="checked"' : '').'/>';
        }
...
EOT;
    ...
    public function showprofilesettings()
    {
        ...
        if ($JAX->p['submit']) {
            ...
            if ($CFG['badnamechars'] && preg_match($CFG['badnamechars'], $data['display_name'])) {
            ...
            if ($data['dob_month'] || $data['dob_year'] || $data['dob_day']) {
                ...
                if (($data['dob_month'] % 2) && 31 == $data['dob_day']
                    || 2 == $data['dob_month'] && (!$data['dob_year'] % 4 && $data['dob_day'] > 29 || $data['dob_year'] % 4 && $data['dob_day'] > 28)
                ) {
            ...
            foreach (array(
                ...
                if (false !== strstr($k, 'contact') && preg_match('/[^\\w.@]/', $data[$k])) {
        ...
        foreach (array('', 'male', 'female') as $v) {
            $sexselect .= '<option value="'.$v.'"'.($data['sex'] == $v ? ' selected="selected"' : '').'>'.$JAX->pick(ucfirst($v), 'Not telling').'</option>';
        }
        $sexselect .= '</select>';
        ...
        foreach (array('January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December') as $k => $v) {
            $dobselect .= '<option value="'.($k + 1).'"'.(($k + 1) == $data['dob_month'] ? ' selected="selected"' : '').'>'.$v.'</option>';
        }
        $dobselect .= '</select><select name="dob_day"><option value="">--</option>';
        for ($x = 1; $x < 32; ++$x) {
            $dobselect .= '<option value="'.$x.'"'.($x == $data['dob_day'] ? ' selected="selected"' : '').'>'.$x.'</option>';
        }
        $dobselect .= '</select><select name="dob_year"><option value="">--</option>';
        $thisyear = (int) date('Y');
        for ($x = $thisyear; $x > $thisyear - 100; --$x) {
            $dobselect .= '<option value="'.$x.'"'.($x == $data['dob_year'] ? ' selected="selected"' : '').'>'.$x.'</option>';
        }
        $dobselect .= '</select>';
    ...
    public function showboardsettings()
    {
        ...
        $result = (2 != $USER['group_id']) ? $DB->safeselect('*', 'skins', 'WHERE hidden!=1 ORDER BY `title` ASC') :
        $DB->safeselect('*', 'skins', 'ORDER BY `title` ASC');
        $select = '';
        while ($f = $DB->row($result)) {
            $select .= "<option value='".$f['id']."' ".($USER['skin_id'] == $f['id'] ? "selected='selected'" : '').'/>'.($f['hidden'] ? '*' : '').$f['title'].'</option>';
            $found = true;
        }
    ...
    public function viewmessage($messageid)
    {
        ...
        $result = $DB->safespecial(
            'SELECT a.*,m.group_id,m.display_name name,m.avatar,m.usertitle FROM %t a LEFT JOIN %t m ON a.from=m.id WHERE a.id=? ORDER BY date DESC',
    ...
    public function viewmessages($view = 'inbox')
    {
        ...
        if ('sent' == $view) {
            $result = $DB->safespecial(
                'SELECT a.*,m.display_name FROM %t a LEFT JOIN %t m ON a.to=m.id WHERE a.from=? AND !del_sender ORDER BY a.date DESC',
                array('messages', 'members'),
                $USER['id']
            );
        } elseif ('flagged' == $view) {
            $result = $DB->safespecial(
                'SELECT a.*,m.display_name FROM %t a LEFT JOIN %t m ON a.from=m.id WHERE a.to=? AND !del_recipient AND flag=1 ORDER BY a.date DESC',
                array('messages', 'members'),
                $USER['id']
            );
        } else {
            $result = $DB->safespecial(
                'SELECT a.*,m.display_name FROM %t a LEFT JOIN %t m ON a.from=m.id WHERE a.to=? AND !del_recipient ORDER BY a.date DESC',
                array('messages', 'members'),
                $USER['id']
            );
        }
        ...
        if (!$hasmessages) {
            if ('sent' == $view) {
                ...
                $msg = 'No messages. You could always try <a href="?act=ucp&what=inbox&page=compose">sending some</a>, though!';
            }
            $page .= '<tr><td colspan="5" class="error">'.$msg.'</td></tr>';
        }

        $page = $PAGE->meta('inbox-messages-listing', $JAX->hiddenFormFields(array('act' => 'ucp', 'what' => 'inbox')), 'sent' == $view ? 'Recipient' : 'Sender', $page);
    ...
    public function compose($messageid = '', $todo = '')
    {
        ...
        if ($JAX->p['submit']) {
            ...
            if ($e) {
                ...
                $result = $DB->safespecial(
                    'UPDATE %t SET runonce=concat(runonce,?) WHERE uid=?',
                    array('session'),
                    $DB->basicvalue($cmd, 1),
                    $udata['id']
                );
     ...
     Please go to <a href='{BOARDURL}?act=ucp&what=inbox'>{BOARDURL}?act=ucp&what=inbox</a> to view your message."
        ...
        if ($messageid) {
            $result = $DB->safeselect(
                '*', 'messages', 'WHERE (`to`=? OR `from`=?) AND `id`=?',
                $USER['id'],
                $USER['id'],
                $DB->basicvalue($messageid)
            );
    ...
    public function delete($id, $relocate = true)
    {
        ...
        if ($is_recipient) {
            $DB->safeupdate('messages', array('del_recipient' => 1), 'WHERE id=?', $DB->basicvalue($id));
        }
        if ($is_sender) {
            $DB->safeupdate('messages', array('del_sender' => 1), 'WHERE id=?', $DB->basicvalue($id));
        }
        $result = $DB->safeselect('*', 'messages', 'WHERE `id`=?', $DB->basicvalue($id));
        $message = $DB->row($result);
        $DB->disposeresult($result);
        ...
        if ($message['del_recipient'] && $message['del_sender']) {
            $DB->safedelete('messages', 'WHERE id=?', $DB->basicvalue($id));
        }
        if ($relocate) {
            $PAGE->location('?act=ucp&what=inbox'.($JAX->b['prevpage'] ? '&page='.$JAX->b['prevpage'] : ''));
        }
    }
}
