...
class FORUM
{
    ...
    public function __construct()
    {
        ...
        $this->numperpage = 20;
        $this->page = 0;
        if (is_numeric($JAX->b['page']) && $JAX->b['page'] > 0) {
        ...
        if ($PAGE->jsupdate) {
            $this->update();
        } elseif (is_numeric($JAX->b['replies'])) {
    ...
    array('forums', 'members'));
     ...
     $read ? $JAX->pick($PAGE->meta('subforum-icon-read'), $PAGE->meta('icon-read')) : $JAX->pick($PAGE->meta('subforum-icon-unread'), $PAGE->meta('icon-unread'))
        ...
        if ($rows) {
            $page .= $PAGE->collapsebox('Subforums', $PAGE->meta('forum-subforum-table', $rows));
        }
        ...
        if ($numpages) {
            foreach ($JAX->pages($numpages, $this->page + 1, 10) as $v) {
                $forumpages .= '<a href="?act=vf'.$fid.'&amp;page='.$v.'"'.(($v - 1) == $this->page ? ' class="active"' : '').'>'.$v.'</a> ';
            }
        }
        ...
        while ($f = $DB->row($result)) {
            ...
            if ($f['replies'] > 9) {
                foreach ($JAX->pages(ceil(($f['replies'] + 1) / 10), 1, 10) as $v) {
                    $pages .= "<a href='?act=vt".$f['id']."&amp;page=${v}'>${v}</a> ";
                }
    ...
    $read ? $JAX->pick($PAGE->meta('topic-icon-read'), $PAGE->meta('icon-read')) : $JAX->pick($PAGE->meta('topic-icon-unread'), $PAGE->meta('icon-read')) //14
        ...
        if ($rows) {
            ...
            if ($fdata['perms']['start']) {
                $table = $PAGE->error("This forum is empty! Don't like it? <a href='?act=post&amp;fid=".$fid."'>Create a topic!</a>");
            }
        }
    ...
    public function isTopicRead($topic, $fid)
    {
        ...
        if ($topic['lp_date'] > $JAX->pick(max($this->topicsRead[$topic['id']], $this->forumReadTime), $SESS->readtime, $USER['last_visit'])) {
            return false;
        }
    ...
    public function isForumRead($forum)
    {
        ...
        if ($forum['lp_date'] > $JAX->pick($this->forumsRead[$forum['id']], $SESS->readtime, $USER['last_visit'])) {
            return false;
        }
