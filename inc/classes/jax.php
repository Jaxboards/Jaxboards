...
class JAX
{
    ...
    public static function json_encode($a, $forceaa = false)
    {
        ...
        if ($forceaa || array_diff($keys, range(0, count($a)))) {
            ...
            foreach ($a as $v) {
                $r .= (is_array($v) ? self::json_encode($v) : '"'.str_replace(array_keys($replaces), array_values($replaces), $v).'"').',';
            }
            if (',' == substr($r, -1)) {
                $r = substr($r, 0, -1);
            }
    ...
    public function getTextRules()
    {
        ...
        foreach ($textRules['emote'] as $k => $v) {
            $nrules[preg_quote($k, '@')] = '<img src="'.$v.'" alt="'.$this->blockhtml($k).'"/>';
        }
    ...
    public function getEmoteRules($escape = 1)
    {
        ...
        if (file_exists(BOARDPATH.'emoticons.php')) {
            ...
            foreach ($emoticons as $k => $v) {
                $DB->safeinsert('textrules', array('type' => 'emote', 'needle' => $k, 'replacement' => $v));
            }
            unlink(BOARDPATH.'emoticons.php');
            foreach ($emoticons as $k => $v) {
                $nrules[($escape ? preg_quote($k, '@') : $k)] = '<img src="'.$v.'" />';
            }
        }
        $this->getTextRules();
    ...
    {
        while (($t = preg_replace('@\\[(\\w+)[^\\]]*\\]([\\w\\W]*)\\[/\\1\\]@U', '$2', $a)) != $a) {
            $a = $t;
        }
    ...
    public function bbcodes($a, $minimal = false)
    {
        ...
        //ul/li tags
        while ($a != ($tmp = preg_replace_callback('@\\[(ul|ol)\\](.*)\\[/\\1\\]@Usi', array($this, 'bbcode_licallback'), $a))) {
            $a = $tmp;
        }
        //size code (actually needs a callback simply because of the variability of the arguments)
        while ($a != ($tmp = preg_replace_callback('@\\[size=([0-4]?\\d)(px|pt|em|)\\](.*)\\[/size\\]@Usi', array($this, 'bbcode_sizecallback'), $a))) {
            $a = $tmp;
        }
        ...
        while (preg_match('@\\[quote(?>=([^\\]]+))?\\](.*?)\\[/quote\\]\\r?\\n?@is', $a, $m) && $x < 10) {
            ++$x;
            $a = str_replace($m[0], '<div class="quote">'.($m[1] ? '<div class="quotee">'.$m[1].'</div>' : '').$m[2].'</div>', $a);
        }
   ...
   case 'profile_comment':
    if ($rssversion) {
        $r = array('text' => $a['name'].' commented on '.$a['aff_name']."'s profile", 'link' => '?act=vu'.$a['aff_id']);
    } else {
   ...
   case 'new_post':
    if ($rssversion) {
        $r = array('text' => $a['name'].' posted in topic '.$a['arg1'], 'link' => '?act=vt'.$a['tid'].'&findpost='.$a['pid']);
    } else {
        $r = $user.' posted in topic <a href="?act=vt'.$a['tid'].'&findpost='.$a['pid'].'">'.$a['arg1'].'</a>, '.$this->smalldate($a['date']);
    }
   break;
   case 'new_topic':
    if ($rssversion) {
        $r = array('text' => $a['name'].' created new topic '.$a['arg1'], 'link' => '?act=vt'.$a['tid']);
    } else {
        $r = $user.' created new topic <a href="?act=vt'.$a['tid'].'">'.$a['arg1'].'</a>, '.$this->smalldate($a['date']);
    }
   break;
   case 'profile_name_change':
    if ($rssversion) {
        $r = array('text' => $a['arg1'].' is now known as '.$a['arg2'], 'link' => '?act=vu'.$a['uid']);
    } else {
        $r = $PAGE->meta('user-link', $a['uid'], $a['group_id'], $a['arg1']).' is now known as '.$PAGE->meta('user-link', $a['uid'], $a['group_id'], $a['arg2']).', '.$this->smalldate($a['date']);
    }
   break;
   case 'buddy_add':
    if ($rssversion) {
        $r = array('text' => $a['name'].' made friends with '.$a['aff_name'], 'link' => '?act=vu'.$a['uid']);
    } else {
   ...
   break;
    ...
    public function ipbanned($ip = false)
    {
        ...
        foreach ($this->ipbancache as $v) {
            if ('.' == substr($v, -1) && substr($ip, 0, strlen($v)) == $v) {
                return $v;
            }
            if ($v == $ip) {
                return $v;
    ...
    public function forumspammer($ip = false)
    {
        if (!$ip) {
            $ip = $_SERVER['REMOTE_ADDR'];
        }
    ...
    public function toruser($ip = false)
    {
        if (!$ip) {
            $ip = $_SERVER['REMOTE_ADDR'];
        }
        ...
        return '127.0.0.2' == gethostbyname($name);
    }

    public function ip2int($ip = false)
    {
        if (!$ip) {
            $ip = $_SERVER['REMOTE_ADDR'];
        }
    ...
    public function parseperms($permstoparse, $uid = false)
    {
        ...
        if (null === $permstoparse) {
            return array('upload' => $PERMS['can_attach'], 'reply' => $PERMS['can_post'], 'start' => $PERMS['can_post_topics'], 'read' => 1, 'view' => 1, 'poll' => $PERMS['can_poll']);
        }
