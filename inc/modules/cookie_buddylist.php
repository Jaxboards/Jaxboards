...
new buddylist;
class buddylist{
 function buddylist(){$this->__construct();}
 function __construct(){
  global $PAGE,$JAX,$USER;
  if(!$USER) {$PAGE->JS("softurl");if($JAX->c['buddylist']) return setcookie("buddylist",false);return $PAGE->JS("error","Sorry, you must be logged in to use this feature.");}
  if($JAX->b['add']) $this->addbuddy($JAX->b['add']);
  elseif($JAX->b['remove']) $this->dropbuddy($JAX->b['remove']);
  elseif($JAX->b['status']) $this->setstatus($JAX->b['status']);
  elseif($JAX->b['block']) $this->block($JAX->b['block']);
  elseif($JAX->b['unblock']) $this->unblock($JAX->b['unblock']);
  elseif($JAX->b['module']=="buddylist") $this->displaybuddylist();
  else $this->update();
 }
 ...
 function displaybuddylist(){
  ...
  if(!$crap) $crap=$PAGE->meta('error',"You don't have any contacts added to your buddy list!");
 ...
 function addbuddy($uid){
  ...
  if($USER['enemies']&&in_array($uid,explode(",",$USER['enemies']))) $this->unblock($uid);
  ...
  elseif(in_array($uid,explode(",",$friends))) $e="This user is already in your contacts list.";
 ...
 function block($uid){
  ...
  if($isfriend!==false) $this->dropbuddy($uid,1);
  if($isenemy!==false) $e="This user is already blocked.";
  if($e) {
   $PAGE->JS("error",$e);
  } else {
   $enemies[]=$uid;
